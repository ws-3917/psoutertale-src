...
import { BLEND_MODES, ColorMatrixFilter, Graphics, isMobile, Rectangle } from 'pixi.js';
...
export const hyper = {
    ...
    color_update(color: HyperColor) {
        ...
        if (color === 'purple') {
            if (SAVE.data.b._overrideStat || mechanics.base_inv !== 30) {
                battler.stat.invulnerability.modifiers = [['multiply', 1.5, Infinity]];
            } else {
                ...
                if (time < 105) {
                    ...
                    if (time === 105) {
                        if (this.position.extentOf(battler.SOUL) < 18) {
                            ...
                            // WS3917 - this modification can make sure green soul heal won't cancel the INF hp
...
export const hypervisor = {
    ...
    load(slot: number) {
        ...
        // WS3917 - this modification can keep the INF hp for debug
        if (SAVE.data.n.hp !== Infinity) {
                ...
                while (true) {
                    ...
                    if (hy.active && hypervisor.phase === 6) {
                        ...
                        const safe = {
                            ...
                            tick() {
                                const timeLimit = isMobile.any ? 35 : 30;
                                if (hypervisor.time === timeLimit * 30) {
                                    safe.end?.confirm();
                                } else {
                                    const safe_value =
                                    ...
                                    if (safe.mus_daemon.instances[0]) {
                                        safe.mus_daemon.instances[0].rate.value = (192 / 246.857) * (30 / timeLimit);
                                    }
                                    const v1 = Math.max((hypervisor.time - 25 * 30) / (30 * (timeLimit - 25)), 0);
                                    musicFilter.value = v1 * 0.6;
                                    musicConvolver.value = v1 * 0.8;
                                    renderer.speed.value = 1 - v1 * 0.2;
                                    if (hypervisor.time < (timeLimit - 20) * 30) {
                                        safe.fg.alpha.value = 0;
                                        safe.load_text.metadata.ticks = 30;
                                    } else {
                                        safe.fg.alpha.value =
                                            ...
                                            ((hypervisor.time - (timeLimit - 20) * 30) / 600);
                                        safe.load_text.content = text.a_citadel.hypertext.count.replace(
                                            '$(x)',
                                            (timeLimit - Math.floor(hypervisor.time / 30)).toString().padStart(2, '0')
                                        );
