...
import { atlas, events, game, renderer, rng, speech } from '../systems/core';
import {
    battler,
    box,
    calcLVX,
...
const patterns = {
    async alphys(t: number): Promise<void> {
        if (SAVE.data.b._overrideStat || mechanics.base_inv !== 30) {
            battler.stat.invulnerability.modifiers.push(['multiply', 1 - (calcLVX() - 1) / 40, 1]);
        } else {
