...
export const friskMirror = {
    right: content.iocFriskRight,
    left: content.iocFriskLeft,
    up: content.iocFriskDown,
    down: content.iocFriskUp
};

export const friskMirrorWater = {
    right: content.iocFriskRightWater,
    left: content.iocFriskLeftWater,
    up: content.iocFriskDownWater,
    down: content.iocFriskUpWater
};

export const friskMirrorHome = {
    right: content.iocFriskRightHome,
    left: content.iocFriskLeftHome,
    up: content.iocFriskDownHome,
    down: content.iocFriskUpHome
};

export const friskRun = {
    down: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunDown }),
    left: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunLeft }),
    right: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunRight }),
    up: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunUp })
};

export const friskRunMirrorWater = {
    right: content.iocFriskRunRightWater,
    left: content.iocFriskRunLeftWater,
    up: content.iocFriskRunDownWater,
    down: content.iocFriskRunUpWater
};

export const friskRunMirrorHome = {
    right: content.iocFriskRunRightHome,
    left: content.iocFriskRunLeftHome,
    up: content.iocFriskRunDownHome,
    down: content.iocFriskRunUpHome
};

export const friskRunMirror = {
    right: content.iocFriskRunRight,
    left: content.iocFriskRunLeft,
    up: content.iocFriskRunDown,
    down: content.iocFriskRunUp
};

export const friskRunMirrorArchive = {
    right: content.iocFriskRunRightArchive,
    left: content.iocFriskRunLeftArchive,
    up: content.iocFriskRunDownArchive,
    down: content.iocFriskRunUpArchive
};

export const friskRunWater = {
    down: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunDownWater }),
    left: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunLeftWater }),
    right: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunRightWater }),
    up: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunUpWater })
};

export const friskRunWaterArchive = {
    down: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunDownWaterArchive }),
    left: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunLeftWaterArchive }),
    right: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunRightWaterArchive }),
    up: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunUpWaterArchive })
};

export const friskRunHome = {
    down: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunDownHome }),
    left: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunLeftHome }),
    right: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunRightHome }),
    up: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunUpHome })
};

export const friskRunArchive = {
    down: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunDownArchive }),
    left: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunLeftArchive }),
    right: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunRightArchive }),
    up: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRunUpArchive })
};
...
// WS3917 - fix running problem for normal undyne (not helmet)
export const erndyne = new CosmosCharacter({
    ...
    if (game.movement) {
        this.metadata.move > 0 && (this.metadata.move -= game.mspeed_last * (0.25 + (player.metadata.moved ? 0 : 1)));
        this.metadata.move < 0 && (this.metadata.move = 0);
    }
    const animate = !this.talk && !this.metadata.static;
    // WS3917 - trying to use the value to adjust chasing path delay
    const to = tracker.interpolate(6 / game.mspeed_last + this.metadata.move)[1];
    this.face = ultimaFacer(this);
    if (this.position.x !== to.x || this.position.y !== to.y) {
        this.position.set(to);
        if (animate) {
            // WS3917 - change the animation duration based on movement speed
            const duration = Math.round(5 / Math.max(Math.sqrt(game.mspeed_last), 1));
            for (const sprite of Object.values(this.sprites)) {
                ...
                new CosmosObject({
                    ...
                    objects: [
                        ...
                        new CosmosAnimation({ anchor: { x: 0, y: 1 } }).on('pre-render', function () {
                            ...
                            this.use(game.mspeed > 1 ? friskRunMirrorHome[player.face] : friskMirrorHome[player.face]);
...
events.on('script', async (name, ...args) => {
    ...
    switch (name) {
        ...
        case 'save':
            ...
            if (
                (world.population === 0 && (world.genocide || !world.bullied)) ||
                (game.room[0] === 'w' && world.genocide)
            ) {
...
player.on('tick', function () {
    ...
    // WS3917 - use game.room to decide sprites rather than freedom variable
    // to avoid sprite missing in debug testing (maybe)
    const home = SAVE.data.b.freedom; // game.room.includes('_frontier');
    ...
    const mSpeed = game.mspeed;
    if (mSpeed !== this.metadata.cache.mspeed) {
        this.metadata.cache.mspeed = mSpeed;
        dirty = true;
    }
    if (dirty) {
        if (home) {
            this.sprites = mSpeed > 1 ? friskRunHome : friskHome;
        } else if (water) {
            this.sprites = archive
                ? mSpeed > 1
                    ? friskRunWaterArchive
                    : friskWaterArchive
                : mSpeed > 1
                    ? friskRunWater
                    : friskWater;
        } else {
            this.sprites = archive ? (mSpeed > 1 ? friskRunArchive : friskArchive) : mSpeed > 1 ? friskRun : frisk;
        }
    }
