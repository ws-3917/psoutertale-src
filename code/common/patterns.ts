...
import { battler, box, keyState, quickshadow, shake, sineWaver, world } from '../systems/framework';
            ...
            if (this.metadata.active) {
                if (!this.metadata.trig1) {
                    if (
                        ...
                        // WS3917 - this modification could make the trigger a gap rather than a solid line
                        const toleranceDistance = 7; // 12
                        if (
                            (color === 'yellow'
                                ? spawnPosition.extentOf(this) < distanceToCenter + 14 - toleranceDistance
                                : spawnPosition.extentOf(this) > distanceToCenter - 14 + toleranceDistance) &&
                            (circular || Math.abs(x - cage.position.x) < 13)
                        ) {
                ...
                // WS3917 - I changed this value as well to match the tolarance gap modificaion
                const missDistance = circular || Math.abs(x - battler.SOUL.position.x) < 4 ? 2 : 1;
...
const directQueue = [] as CosmosDirection[];
export const undKeyHandler = (cage: CosmosObject, vars = battler.volatile[0].vars) =>
    async function () {
        // modify the state to make it more "reasonable" (WS3917)
        if (!undState.cooldown) {
            const curDur = vars.shield;
            if (curDur) {
                let newDir: CosmosDirection;
                const directStateList = [keyState.up, keyState.left, keyState.right, keyState.down];
                const directNameList = ['up', 'left', 'right', 'down'] as CosmosDirection[];
                directStateList.forEach((pressed, index) => {
                    if (pressed) {
                        // new key
                        if (!directQueue.includes(directNameList[index])) {
                            directQueue.push(directNameList[index]);
                        }
                    } else {
                        if (directQueue.includes(directNameList[index])) {
                            directQueue.splice(directQueue.indexOf(directNameList[index]), 1);
                        }
                    }
                });
                if (directQueue.length === 0) {
                    ...
                    newDir = directQueue[directQueue.length - 1];
                }
                if (curDur !== newDir) {
