...
import './bootstrap';
import { armorprice, ghostpartyCondition, startonATE, temgone, undyneMusicAssets } from './extras';
                    ...
                    if (roomState.mode === 'transition') {
                        if (proximity) {
                            ...
                            if (diffX > 0 || diffY > 0) {
                                ...
                                // WS3917 - Trying to solve this problem
                                kiddo.move(
                                    {
                                        x: (diffX === 0 ? 0 : diffX < 4 * game.mspeed_last ? diffX : 4 * game.mspeed_last) * dirX,
                                        y: (diffY === 0 ? 0 : diffY < 4 * game.mspeed_last ? diffY : 4 * game.mspeed_last) * dirY
                                    },
                                    renderer
                                );
                            } else {
                            ...
                            const modRate = 4.5 * game.mspeed_last;
                            if (extent <= 21 * game.mspeed_last + modRate) {
                                kiddo.position.set(
        ...
        switch (subscript) {
            ...
            case 'fakedummy': {
                ...
                if (SAVE.data.b.f_state_dummypunch || SAVE.data.b.f_state_dummyhug) {
                    ...
                    if (choicer.result === 0) {
                        ...
                        if (
                            !(world.genocide || world.meanie || SAVE.data.n.exp > 0 || SAVE.data.b.oops) &&
                            25 <= world.flirt
                        ) {
                ...
                if (helmetdyne()) {
                    ...
                    const fish2 = temporary(
                        character('fish2', characters.undyneArmor, { x: 400, y: 220 }, 'right', {
                            ...
                            const tempspd = 4 * Math.max(1, game.mspeed_last);
                            if (Math.abs(player.position.x - this.position.x) <= 40) {
                                this.move({
                                    x: Math.min(player.position.x - this.position.x, tempspd),
                                    y: Math.min(player.position.y - this.position.y, tempspd)
                                });
                                if (this.position.extentOf(player) < 20) {
                                    game.movement = false;
                                    groups.undyne.assets = undyneMusicAssets();
                                ...
                                // WS3917 - make the undyne chasing speed sync to play move speed
                                // so that it's always faster than player.
                                this.position.x += tempspd;
                                if (this.position.x < 440) {
                                    this.sprite.enable();
                                } else if (this.position.x < 460) {
                                    this.position.y -= tempspd;
                                    if (this.face === 'right') {
                                        this.sprite.reset();
                                        this.face = 'up';
                                        this.sprite.enable();
                            ...
                            // WS3917 - I use Square root to make the effect less intense
                            // default tempspd = 4, 7.5 / 2 = 15 / 4, it doesn't change original duration
                            this.sprite.duration = Math.round(7.5 / Math.sqrt(tempspd));
            ...
            case 'fallenfish': {
                ...
                if (SAVE.data.b.water) {
                    ...
                    player.face = 'right';
                    player.position.x += 3 * Math.max(1, game.mspeed_last);
                }
                break;
            }
            case 'fallenfish2': {
        ...
        price() {
            if (shopper.index === 0) {
                return SAVE.data.n.plot === 72
                    ...
                    : temgone()
                        ? [0, 0, 0, Infinity][shopper.listIndex]
