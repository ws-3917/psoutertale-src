...
import { gameBorder } from './systems/border';
import * as core from './systems/core';
import * as developer from './systems/developer';
import * as framework from './systems/framework';
import './systems/mobile';
import { mobileConfigBtns, mobileDebugBtn, mobileFullScrBtn, mobileResetBtn } from './systems/mobile';
...
export async function mobileReset() {
    if (
        (await dialog.message(false, 'confirm', {
            buttons: systemsText.dialog.message_confirm,
            title: systemsText.dialog.dialog_clear_mobile,
            message: `${systemsText.dialog.prompt_clear_mobile}`
        })) === 1
    ) {
        saver.time_but_real.active = false;
        for (const key of SAVE.keys()) {
            key !== 'ERRORCODE' && key.includes('$option_mobile') && SAVE.manager.removeItem(key);
        }
        backend?.writeSave(SAVE.serialize());
        param('namespace');
        reload();
...
renderer.on('tick', {
    ...
    listener() {
        // WS3917 - for testing purpose, remove later
        // if (saver.time % 120 === 0)
        //     keys.interactKey.down('KeyZ');
        // else
        //     keys.interactKey.up('KeyZ');
        if (inputState.hyper()) {
            keyState.down = false;
...
addEventListener('keydown', event => {
    switch (event.code) {
        ...
        case 'F11':
            ...
            break;
        case 'F12':
            // WS3917 - Avoid accidentally opening devtools
            //        case 'Backslash':
            if (panel.active) {
...
(async () => {
    ...
    game.resize();
    if (isMobile.any) {
        if (mobileDebugBtn && mobileFullScrBtn && mobileResetBtn) {
            mobileDebugBtn.onPress = () => panel.activate();
            mobileFullScrBtn.onPress = () => backend?.exec('f4') ?? fullscreen();
            mobileResetBtn.onPress = () => mobileReset();
        }
        if (mobileConfigBtns) {
            mobileConfigBtns.forEach((btn, index) => {
                btn.onPress = () => {
                    framework.frontEnder.mobileSaveConfig();
                    SAVE.flag.n.$option_mobileConfigIndex = index;
                    framework.frontEnder.mobileLoadConfig();
                };
            });
        }
        framework.frontEnder.mobileLoadConfig();
    ...
    translatorImages.updateMode(SAVE.flag.s.$option_colorMode || 'default');
    ...
    SAVE.flag.n.$option_border = Math.min(borderList.length - 1, SAVE.flag.n.$option_border);
