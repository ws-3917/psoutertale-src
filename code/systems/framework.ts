...
import {
    ...
    mobileDefaultCfg,
    mobileDiagBtns,
    mobileDisableBtninput,
    mobileEnableBtninput,
    mobileFullScrBtn,
    mobileFuncBtns,
    mobileJoystk,
    mobileJoystk0,
...
// SAVE.flag.s.$gamepad_input_f = '123';
// isMobile.any = true;
...
export const battler = {
    ...
    stat: {
        ...
        invulnerability: new OutertaleStat(() => {
            if (SAVE.data.b._overrideStat) {
                return SAVE.data.n._INV;
            }
            return Math.max(
...
export const choicer = {
    ...
    options: [] as string[],
    /** create choicer */
    create(header: string, ...options: string[]) {
        let riseline = 0;
        const formattedHeader = values.textFormat(header, 32);
        const optionsString = options
            .map((option, index) => {
                const suffix = index % 2 === 1 ? '\n' : '';
                if (option.split('\n').length > 1) {
                    riseline = Math.max(riseline, option.split('\n').length - 1);
                }
                return `/${option}${suffix}`;
            })
            .join('');
        return `{#p/human}${formattedHeader}{#c/${riseline}${optionsString}}`;
    }
};
...
export const frontEnder = {
    ...
    mobileLoadDefaultConfig() {
        /**
           Default Config:
           - Config 0: Joystick on the left side;
           - Config 1: Joystick on the left side; (Left move ZXC so that they won't cover debug menu)
           - Config 2: 4 Arrow Keys on the left side;
           - Config 3: 8 Arrow Keys on the left side;
           - Config 4: Joystick on the right side;
           */
        const configData = SAVE.flag.s.$option_mobileConfig.split('/');
        SAVE.flag.b.$option_mobileEnableDiagonal = SAVE.flag.n.$option_mobileConfigIndex === 2 ? false : true;
        SAVE.flag.b.$option_mobileShowFullScrKey = true;
        SAVE.flag.n.$option_mobileControlType =
            SAVE.flag.n.$option_mobileConfigIndex === 2 || SAVE.flag.n.$option_mobileConfigIndex === 3 ? 1 : 0;
        SAVE.flag.n.$option_mobileOpacity = 0.8;
        SAVE.flag.s.$option_mobileKeyPosition = mobileDefaultCfg;
        configData[SAVE.flag.n.$option_mobileConfigIndex] = [
            SAVE.flag.b.$option_mobileEnableDiagonal ? '1' : '0',
            SAVE.flag.b.$option_mobileShowFullScrKey ? '1' : '0',
            SAVE.flag.n.$option_mobileControlType.toString(),
            SAVE.flag.n.$option_mobileOpacity.toString(),
            SAVE.flag.s.$option_mobileKeyPosition
        ].join(':');
        SAVE.flag.s.$option_mobileConfig = configData.join('/');
        this.updateMobileConfig();
        if (SAVE.flag.n.$option_mobileConfigIndex === 4) {
            this.mobileInvertButtonPos();
        }
        if (SAVE.flag.n.$option_mobileConfigIndex === 1) {
            [...mobileFuncBtns].forEach(btn => {
                btn.setPos(-30, 0);
            });
        }
    },
    mobileSaveConfig() {
        ...
        configData[SAVE.flag.n.$option_mobileConfigIndex] = tempData;
        SAVE.flag.s.$option_mobileConfig = configData.join('/');
    },
    mobileLoadConfig() {
        const config = SAVE.flag.s.$option_mobileConfig.split('/')[SAVE.flag.n.$option_mobileConfigIndex];
        if (config === '') {
            this.mobileLoadDefaultConfig();
        } else {
            const part = config.split(':');
            SAVE.flag.b.$option_mobileEnableDiagonal = part[0] === '1' ? true : false;
            SAVE.flag.b.$option_mobileShowFullScrKey = part[1] === '1' ? true : false;
            SAVE.flag.n.$option_mobileControlType = parseInt(part[2]);
            SAVE.flag.n.$option_mobileOpacity = parseFloat(part[3]);
            SAVE.flag.s.$option_mobileKeyPosition = part[4];
            this.updateMobileConfig();
        }
    },
    updateMobileConfig() {
        this.updateDiagonalButton();
        this.updateShowFullScrKey();
        this.updateType();
        this.updatemobileOpacity();
        this.updateKeyPos();
        this.updateConfigKey();
    },
    mobileInvertButtonPos() {
        [...mobileFuncBtns, ...mobileArrowBtns, ...mobileDiagBtns].forEach(btn => {
            btn.setPosAbsolute(
                Math.round(btn.relative ? 100 * Math.max(window.innerWidth / window.innerHeight, 1) - btn.x : 100 - btn.x),
                btn.y
            );
        });
        [mobileArrowBtns[0], mobileDiagBtns[0], mobileDiagBtns[3]].forEach(btn => {
            btn.setPos(-mobileArrowGroup.arrowradius * 2, 0);
        });

        [mobileArrowBtns[2], mobileDiagBtns[1], mobileDiagBtns[2]].forEach(btn => {
            btn.setPos(mobileArrowGroup.arrowradius * 2, 0);
        });
        if (mobileJoystk0) {
            mobileJoystk0.setPosAbsolute(
                Math.round(100 * Math.max(window.innerWidth / window.innerHeight, 1) - mobileJoystk0.x),
                mobileJoystk0.y
            );
        }
    },
    updateKeyPos() {
        const positionPart = SAVE.flag.s.$option_mobileKeyPosition.split('|');
        if (mobileJoystk0) {
            mobileUtils.loadJoystkConfig(mobileJoystk0, positionPart[0]);
        }
        [...mobileFuncBtns, ...mobileArrowBtns, ...mobileDiagBtns].forEach((btn, index) =>
            mobileUtils.loadKeyConfig(btn, positionPart[1 + index])
        );
    },
    updateType() {
        if (mobileJoystk0) {
            if (SAVE.flag.n.$option_mobileControlType === 0) {
                if (mobileJoystk0) {
                    mobileJoystk0.canvas.style.display = 'block';
                }
                ...
                if (mobileJoystk0) {
                    mobileJoystk0.canvas.style.display = 'none';
                }
                mobileArrowBtns.forEach(btn => {
    ...
    updatemobileOpacity() {
        const mobileController = document.querySelector('#mobile-controller') as HTMLElement;
        mobileController.style.opacity = String(SAVE.flag.n.$option_mobileOpacity);
    },
    updateDiagonalButton() {
        if (SAVE.flag.b.$option_mobileEnableDiagonal && SAVE.flag.n.$option_mobileControlType === 1) {
            mobileDiagBtns.forEach(btn => {
                btn.element.style.display = 'block';
            });
        } else {
            mobileDiagBtns.forEach(btn => {
    ...
    updateConfigKey() {
        if (mobileConfigBtns) {
            mobileConfigBtns.forEach(btn => {
                btn.element.style.display = SAVE.flag.b.$option_mobileEnableMultiConfig ? 'block' : 'none';
            });
        }
    },
    updateShowFullScrKey() {
        if (mobileFullScrBtn) {
            if (SAVE.flag.b.$option_mobileShowFullScrKey) {
                mobileFullScrBtn.element.style.display = 'block';
            } else {
                mobileFullScrBtn.element.style.display = 'none';
            }
        }
    },
    closeSettings() {
    ...
    async updateColoredSprite() {
        translatorImages.updateMode(SAVE.flag.s.$option_colorMode);
        this.refreshAssets();
    },
    async refreshAssets() {
        game.input = false;
...
export const player = new CosmosPlayer({
    ...
    metadata: {
        ...
        cache: { room: '', water: false, archive: false, init: false, home: false, mspeed: 1 }
    }
}).on('tick', function () {
...
export const world = {
    ...
    get mttvar() {
        return SAVE.data.b.a_state_hapstablook
            ? SAVE.flag.b.snails
            : !world.badder_lizard
                ? SAVE.flag.b.legs
                : SAVE.flag.b.daggers;
    ...
    set mttvar(v) {
        if (SAVE.data.b.a_state_hapstablook) {
            SAVE.flag.b.snails = v;
        } else if (!world.badder_lizard) {
            SAVE.flag.b.legs = v;
        } else {
            SAVE.flag.b.daggers = v;
        }
    },
    nootflags: {
        add(flag: string) {
...
export function choicerText(
    x: number,
    y: number,
    c: string,
    nav: string,
    navX: CosmosProvider<any>,
    ...
    return new CosmosText({
        ...
        this.fill = menuFinder(nav, navX, navY) ? 0xffff00 : 0xffffff;
    });
}
...
export async function saveSelector() {
    ...
    const saveAtlas: CosmosAtlas<string> = new CosmosAtlas<string>({
        ...
        rename_gamepad: new CosmosNavigator({
            flip: true,
            grid: [
                ...(values.nameLetterMap.length === 0 ? [] : values.nameLetterMap[SAVE.flag.n.$option_namepage]),
                ['quit', 'backspace', 'done']
            ],
            next(self) {
            ...
            objects: [
                ...
                ...(values.nameLetterMap.length === 0
                    ? []
                    : values.nameLetterMap[
                        (SAVE.flag.n.$option_namepage = Math.min(
                            values.nameLetterMap.length - 1,
                            SAVE.flag.n.$option_namepage
                        ))
                    ]
                        .flat()
                        .map((letter, index) => {
                            const { x, y } = values.nameLetterPosition(index, SAVE.flag.n.$option_namepage);
                            return menuText(
                                x,
                                y - 4,
                                () =>
                                    `§fill=${saveAtlas.target === 'rename_gamepad' && saveAtlas.navigator()?.selection() === letter
                                        ? '#ff0'
                                        : '#fff'
                                    }§${letter}`
                            );
                ...
                menuText(
                    ...
                    408 - 4,
                    () =>
                        `§fill=${saveAtlas.target === 'rename_gamepad' && saveAtlas.navigator()?.selection() === 'quit'
                            ? '#ff0'
                            : '#fff'
                        }§${text.menu.name2}`
                ),
                menuText(
                    ...
                    408 - 4,
                    () =>
                        `§fill=${saveAtlas.target === 'rename_gamepad' && saveAtlas.navigator()?.selection() === 'backspace'
                            ? '#ff0'
                            : '#fff'
                        }§${text.menu.name3}`
                ),
                menuText(
                    ...
                    408 - 4,
                    () =>
                        `§fill=${saveAtlas.target === 'rename_gamepad' && saveAtlas.navigator()?.selection() === 'done'
                            ? '#ff0'
                            : '#fff'
                        }§${text.menu.name4}`
                )
...
atlas.navigators.register({
            ...
            new CosmosObject({
                objects: [
                    new CosmosText({
                        ...
                        this.content = `${text.general.lv} ${SAVE.data.b._overrideStat ? SAVE.data.s._LV : calcLV()}`;
                    }),
                    new CosmosObject({
                        fill: 0xffffff,
                        stroke: -1,
                        ...
                        const max = calcHP() === Infinity ? Math.min(cur, 99) : Math.min(calcHP(), 99);
                        const hp =
                            cur === Infinity
                                ? max
                                : calcHP() > 99 && calcHP() !== Infinity
                                    ? Math.min(99, (99 * cur) / calcHP())
                                    : Math.min(99, cur);
                        ...
                        ob3.content = `${cur === Infinity ? text.general.inf : cur.toString().padStart(2, '0')} / ${calcHP()
                            .toString()
                            .padStart(2, '0')}`;
                        ...
                        objects: [
                            ...
                            new CosmosText({
                                ...
                                this.content = world.archive
                                    ...
                                    : `${text.general.lv} ${SAVE.data.b._overrideStat ? SAVE.data.s._LV : calcLV()}`;
                            })
                        ]
                    }),
                    new CosmosObject({
                        ...
                        // display hp
                        const cur = SAVE.data.n.hp;
                        // WS3917 - modify the maximium HP display
                        // If calcHP() exceeds 99, this method can avoid the HP bar from extending beyond the screen
                        // red bar length
                        const max = calcHP() === Infinity ? Math.min(cur, 99) : Math.min(calcHP(), 99);
                        let iv = 0;
                        ...
                        ob4.content = `${battler.fakehp ?? (cur === Infinity ? text.general.inf : cur.toString().padStart(2, '0'))
                            }${world.archive ? '' : ` / ${calcHP().toString().padStart(2, '0')}`}`;
    ...
    choicer: new CosmosNavigator({
        flip: true,
        grid: () => CosmosUtils.populate(Math.ceil(choicer.optioncount / 2), index => [index * 2, index * 2 + 1]),
        next: self => {
        ...
        objects: CosmosUtils.populate(6, index => {
            ...
            const soul = menuSOUL(0, 3, 'choicer', index);
            container.attach(soul);
            return container.on('tick', function () {
                const textContent = choicer.options[index];
                if (this.objects.length < 2) {
                    const text = choicerText(
                        content.ieSOUL.value.width + 4,
                        -18 * choicer.riseline,
                        textContent,
                        'choicer',
                        index
                    ...
                    let text = this.objects[1] as CosmosText;
            ...
            new CosmosObject({
                ...
                objects: [
                    ...
                    menuText(280, 132 - 4, () =>
                        ...
                        this.offsets[0].x =
                            values.loadLVX +
                            (values.textLengthPrecise(SAVE.data.s.name) >= 8
                                ? (values.textLengthPrecise(SAVE.data.s.name) - 8) * 8
                                : 0);
                    }),
                    menuText(498, 132 - 4, () => displayTime(saver.time), {
                        ...
                        this.offsets[0].x =
                            values.loadTimeX +
                            (values.textLengthPrecise(SAVE.data.s.name) >= 12
                                ? (values.textLengthPrecise(SAVE.data.s.name) - 12) * 8
                                : 0);
                    }),
                    ...
                    menuText(
                        ...
                        218 - 4,
                        () =>
                            `§fill=${menuFinder('frontEndLoad', 'continue') ? '#ff0' : '#fff'}§${(SAVE.data.b.freedom && SAVE.flag.b.true_reset) || SAVE.flag.n.lv20 !== 0
                                ? text.menu.load2
                                : text.menu.load1
                            }`
                    ).on('tick', function () {
                    ...
                    menuText(
                        ...
                        218 - 4,
                        () =>
                            `§fill=${menuFinder('frontEndLoad', 'reset')
                                ? '#ff0'
    ...
    frontEndName: new CosmosNavigator<string>({
        ...
        grid: () =>
            values.nameLetterMap.length === 0
                ...
                : [
                    ...
                    ...(values.nameLetterMap.length > 1 ? [values.nameType] : []),
                    ['quit', 'backspace', 'done']
                ],
        next(self) {
            const selection = self.selection() as string;
            switch (selection) {
                ...
                case 'done':
                    return 'frontEndNameConfirm';
                default: {
                    if (values.nameLetterMap.length > 1 && values.nameType.includes(selection)) {
                        SAVE.flag.n.$option_namepage = values.nameType.indexOf(selection);
                    } else {
        ...
        prev() {
            if (values.nameLetterMap.length === 0) {
                frontEnder.name.value = '';
                if (frontEnder.trueReset) {
                    frontEnder.trueReset = false;
                    return 'frontEndLoad';
            ...
            new CosmosObject({
                ...
                objects: [
                    ...
                    menuText(
                        ...
                        118 - 4,
                        () =>
                            `${frontEnder.name.value}${values.textLength(frontEnder.name.value) < 5.5 &&
                    ...
                    new CosmosObject().on('tick', function () {
                        const key = values.nameType.join('');
                        if (this.metadata.key !== key) {
                            this.metadata.key = key;
                            this.objects = values.nameType.map((type, index) =>
                                menuText(
                                    264 + CosmosMath.spread(160, index, values.nameType.length),
                                    408 - 4 - 32,
                                    () => `§fill=${menuFinder('frontEndName', type) ? '#ff0' : '#fff'}§${type}`
                                )
                            );
                        }
                    }),
        ...
        .on('from', function () {
            ...
            if (values.nameLetterMap.length === 0) {
                if (isMobile.any) {
                    if (isMobile.any) {
                        [...mobileFuncBtns, ...mobileArrowBtns, ...mobileDiagBtns].forEach(button => {
                            button.handleTouchEnd();
                        });
                    }
                    const tempname = prompt(text.menu.name1, '') ?? '';
                    // WS3917 - name filter for android
                    const name = tempname
                        .split('')
                        .filter(char => values.nameLetterValidation(char))
                        .join('');
                    frontEnder.name.value = name.length > 6 ? name.substring(0, 6) : name;
                } else {
                    freetext.enable(atlas, {
                        ...
                        update() {
                            while (values.textLength(freetext.value) > 6) {
                                freetext.value =
                                    freetext.value.slice(0, -2) + (freetext.value[freetext.value.length - 1] ?? '');
                            }
            ...
            new CosmosObject({
                ...
                objects: [
                    ...
                    menuText(
                        ...
                        408 - 4,
                        () =>
                            `§fill=${menuFinder('frontEndNameConfirm', 'no') ? '#ff0' : '#fff'}§${values.nameChoiceRestrictions.includes(values.nameValueTranslator(frontEnder.name.value_true))
                                ? text.menu.confirm3
                                : text.general.no
                            }`
                    ).on('tick', function () {
                    ...
                    menuText(
                        ...
                        408 - 4,
                        () =>
                            `§fill=${menuFinder('frontEndNameConfirm', 'yes') ? '#ff0' : '#fff'}§${values.nameChoiceRestrictions.includes(values.nameValueTranslator(frontEnder.name.value_true))
                                ? ''
                                : text.general.yes
                            }`
                    ).on('tick', function () {
    ...
    frontEndSettings: new CosmosNavigator<string>({
        grid: () => [
            ...
            [
                ...
                'coloredsprite',
                ...(SAVE.flag.s.$gamepad_input_f === '' ? [] : ['deadzone']),
                ...(isMobile.any ? ['mobile'] : backend !== null ? ['mods'] : [])
            ]
        ],
        next(self) {
            const selection = self.selection() as string;
            switch (selection) {
                ...
                case 'fancy':
                    ...
                    break;
                case 'coloredsprite':
                    if (translatorImages.modes.length !== 0) {
                        const index = Math.max(translatorImages.modes.indexOf(SAVE.flag.s.$option_colorMode), 0) + 1;
                        SAVE.flag.s.$option_colorMode =
                            translatorImages.modes[index === translatorImages.modes.length ? 0 : index];
                        frontEnder.updateColoredSprite();
                    }
                    break;
                case 'running':
                    SAVE.flag.b.$option_running = !SAVE.flag.b.$option_running;
                    break;
                case 'epilepsy':
        ...
        objects: [
            ...
            new CosmosObject({
                ...
                priority: Number.MAX_SAFE_INTEGER,
                objects: [
                    menuText(320, 36 - 4, () => text.menu.settings1, {
                        fontFamily: content.fDeterminationSans,
                        fontSize: 32
                    }).on('tick', function () {
                        this.offsets[0].x = -values.textLengthPrecise(this.content) * 7;
                    }),
                    menuText(
                        40,
                        84 + 5,
                    ...
                    menuText(40, 84 + 35 + 10, () => `§fill=#0ff§${text.menu.settingsprompt1}`, {
                        fontFamily: content.fDiaryOfAn8BitMage,
                        fontSize: 10
                    }),
                    menuText(
                    ...
                    menuText(320, 84 + 35 * 2, () => {
                        const selected = menuFinder('frontEndSettings', 'language');
                        return `${selected ? '§fill=#ff0§< ' : ''}§fill=${selected ? '#ff0' : '#fff'}§${text.menu.settings3a
                            }${selected ? '§fill=#ff0§ >' : ''}`;
                    }).on('pre-render', function () {
                    ...
                    menuText(320, 84 + 35 * 3, () => {
                        const selected = menuFinder('frontEndSettings', 'sfx');
                        return `${!frontEnder.muteSFX && selected
                            ? `§fill=${frontEnder.volumeSFX === 0 ? '#808000' : '#ff0'}§< `
                            : ''
                            }§fill=${selected ? '#ff0' : '#fff'}§${frontEnder.muteSFX
                                ? text.general.disabled
                                : String.fromCharCode(0x4dd0 + Math.round(frontEnder.volumeSFX * 10))
                    ...
                    new CosmosAnimation({
                        active: true,
                        scale: 1,
                        position: { y: (84 + 35 * 3) / 2 },
                        area: new Rectangle(320, 120, 320, 240),
                        filters: [
                            new OutlineFilter(2, 0xffffff, 0.1, 0.025, false),
                            new OutlineFilter(1, 0xffffff, 0.1, 0.025, false),
                            new AdvancedBloomFilter({ quality: 10, threshold: 0, brightness: 1, bloomScale: 0.1 })
                        ]
                    }).on('tick', function () {
                        this.alpha.value = SAVE.flag.b.$option_fancy ? 0.16 : 0;
                        this.resources = SAVE.flag.b.$option_running ? content.iocFriskRunRight : content.iocFriskRight;
                        this.position.x = (SAVE.flag.b.$option_running ? 432 / 2 : 448 / 2) + 5;
                    }),
                    new CosmosAnimation({
                        active: true,
                        scale: 1,
                        ...
                        area: new Rectangle(320, 120, 320, 240),
                        filters: [
                            new OutlineFilter(2, 0xffffff, 0.1, 0.05, false),
                            new OutlineFilter(1, 0xffffff, 0.1, 0.05, false),
                            new AdvancedBloomFilter({ quality: 10, threshold: 0, brightness: 1, bloomScale: 0.1 })
                        ]
                    }).on('tick', function () {
                        this.alpha.value = SAVE.flag.b.$option_fancy ? 0.4 : 0;
                        this.resources = SAVE.flag.b.$option_running ? content.iocFriskRunRight : content.iocFriskRight;
                        this.position.x = (SAVE.flag.b.$option_running ? 456 / 2 : 464 / 2) + 5;
                    }),
                    new CosmosAnimation({
                        active: true,
                        scale: 1,
                        position: { x: 480 / 2 + 5, y: (84 + 35 * 3) / 2 },
                        area: new Rectangle(320, 120, 320, 240),
                        filters: [
                            new OutlineFilter(2, 0xffffff, 0.1, 0.1, false),
                            new OutlineFilter(1, 0xffffff, 0.1, 0.1, false),
                    ...
                    new CosmosSprite({
                        scale: 0.5,
                        position: { x: 540 / 2, y: (84 + 35 * 3 + 6) / 2 },
                        frames: [content.ibcDummy],
                        area: new Rectangle(320, 120, 320, 240),
                        filters: [
                            new OutlineFilter(2, 0xffffff, 0.1, 0.1, false),
                            new OutlineFilter(1, 0xffffff, 0.1, 0.1, false),
                    ...
                    menuText(320, 84 + 35 * 4, () => {
                        const selected = menuFinder('frontEndSettings', 'music');
                        return `${!frontEnder.muteMusic && selected
                            ? `§fill=${frontEnder.volumeMusic === 0 ? '#808000' : '#ff0'}§< `
                            : ''
                            }§fill=${selected ? '#ff0' : '#fff'}§${frontEnder.muteMusic
                                ? text.general.disabled
                                : String.fromCharCode(0x4dd0 + Math.round(frontEnder.volumeMusic * 10))
                    ...
                    menuText(
                        40,
                        84 + 35 * 5,
                        () => `§fill=${menuFinder('frontEndSettings', 'border') ? '#ff0' : '#fff'}§${text.menu.border.option}`
                    ),
                    menuText(320, 84 + 35 * 5, () => {
                        const selected = menuFinder('frontEndSettings', 'border');
                        return `${selected ? `§fill=${SAVE.flag.n.$option_border === 0 ? '#808000' : '#ff0'}§< ` : ''}§fill=${selected ? '#ff0' : '#fff'
                            }§${text.menu.border.list[gameBorder.getAvaliableBorder()[SAVE.flag.n.$option_border][1] as number]}${selected
                                ? `§fill=${SAVE.flag.n.$option_border === gameBorder.getAvaliableBorder().length - 1
                                    ? '#808000'
                                    : '#ff0'
                                }§ >`
                                : ''
                        ...
                        this.offsets[0].x = menuFinder('frontEndSettings', 'border')
                            ? -content.fDeterminationSans.metrics('< ').x
                            : 0;
                    }),
                    menuText(40, 84 + 35 * 6 + 10, () => `§fill=#0ff§${text.menu.settingsprompt2}`, {
                        fontFamily: content.fDiaryOfAn8BitMage,
                        fontSize: 10
                    }),
                    menuText(
                        ...
                        84 + 35 * 7,
                        () =>
                            `§fill=${menuFinder('frontEndSettings', 'fancy') ? '#ff0' : '#fff'}§${(!SAVE.flag.b.$option_fancy ? '\u4dc4' : '\u4dc5') + text.menu.settings6
                            }`
                    ),
                    menuText(
                        40,
                        84 + 35 * 8,
                        () =>
                            `§fill=${menuFinder('frontEndSettings', 'running') ? '#ff0' : '#fff'}§${(!SAVE.flag.b.$option_running ? '\u4dc4' : '\u4dc5') + text.menu.settings6a
                            }`
                    ),
                    menuText(
                        320,
                        84 + 35 * 7,
                        () =>
                            `§fill=${menuFinder('frontEndSettings', 'epilepsy') ? '#ff0' : '#fff'}§${(SAVE.flag.b.$option_epilepsy ? '\u4dc4' : '\u4dc5') + text.menu.settings7
                            }`
                    ),
                    menuText(
                        320,
                        84 + 35 * 8,
                        () =>
                            `§fill=${menuFinder('frontEndSettings', 'coloredsprite') ? '#ff0' : '#fff'}§${(SAVE.flag.s.$option_colorMode === 'default' ? '\u4dc4' : '\u4dc5') + text.menu.settings6b
                            }`
                    ),
                    ...
                    menuText(320, 84 + 35 * 9, () => {
                        const selected = menuFinder('frontEndSettings', 'deadzone');
                        return `${selected ? `§fill=${gamepadder.deadzone === 0 ? '#808000' : '#ff0'}§< ` : ''}§fill=${selected ? '#ff0' : '#fff'
                            }§${text.general.percent.replace('$(x)', Math.round(100 * gamepadder.deadzone).toString())}${selected ? `§fill=${gamepadder.deadzone === 1 ? '#808000' : '#ff0'}§ >` : ''
                            }`;
                    })
                        .on('tick', function () {
                            this.alpha.value = SAVE.flag.s.$gamepad_input_f === '' ? 0 : 1;
                    ...
                    menuText(
                        320,
                        84 + 35 * 10,
                        () => `§fill=${menuFinder('frontEndSettings', 'mobile') ? '#ff0' : '#fff'}§${text.menu.settings8}`,
                    ...
                    menuText(
                        ...
                        84 + 35 * 10,
                        () =>
                            `§fill=${menuFinder('frontEndSettings', 'mods') ? '#ff0' : '#fff'}§${backend !== null ? text.menu.settings10 : text.menu.settings11
                            }`,
    ...
    mobileSettings: new CosmosNavigator<string>({
        grid: () => [['exit', 'loaddefault', 'multiconfig', 'type', 'opacity', 'invertbutton', 'nextpage']],
        next: self => {
            const selection = self.selection() as string;
            switch (selection) {
                case 'exit':
                    frontEnder.mobileSaveConfig();
                    content.asSave.loaded && sounds.save.instance(renderer);
                    return 'frontEndSettings';
                case 'loaddefault':
                    frontEnder.mobileLoadDefaultConfig();
                    content.asSave.loaded && sounds.save.instance(renderer);
                    break;
                case 'multiconfig':
                    SAVE.flag.b.$option_mobileEnableMultiConfig = !SAVE.flag.b.$option_mobileEnableMultiConfig;
                    frontEnder.updateConfigKey();
                    break;
                case 'type':
                    SAVE.flag.n.$option_mobileControlType = SAVE.flag.n.$option_mobileControlType === 1 ? 0 : 1;
                    frontEnder.updateType();
                    break;
                case 'invertbutton':
                    frontEnder.mobileInvertButtonPos();
                    break;
                case 'nextpage':
                    atlas.switch('mobileSettings2');
                    break;
            }
        },
        prev() {
        ...
        objects: [
            ...
            new CosmosObject({
                ...
                priority: Number.MAX_SAFE_INTEGER,
                objects: [
                    menuText(320, 36 - 4, () => text.menu.mobile.title, {
                        fontFamily: content.fDeterminationSans,
                        fontSize: 32
                    }).on('tick', function () {
                        this.offsets[0].x = -values.textLengthPrecise(this.content) * 7;
                    ...
                    menuText(
                        40,
                        84 + 5,
                        () => `§fill=${menuFinder('mobileSettings', 'exit') ? '#ff0' : '#fff'}§${text.menu.settings2}`
                    ),
                    menuText(40, 84 + 35 * 1 + 10, () => `§fill=#0ff§${text.menu.mobile.prompt1}`, {
                        fontFamily: content.fDiaryOfAn8BitMage,
                        fontSize: 10
                    }),
                    menuText(
                        ...
                        84 + 35 * 2,
                        () =>
                            `§fill=${menuFinder('mobileSettings', 'loaddefault') ? '#ff0' : '#fff'}§${text.menu.mobile.loadDefault
                            }`
                    ),
                    menuText(
                        40,
                        84 + 35 * 3,
                        () =>
                            `§fill=${menuFinder('mobileSettings', 'multiconfig') ? '#ff0' : '#fff'}§${text.menu.mobile.enableMultiConfig
                            }`
                    ),
                    menuText(
                        360,
                        84 + 35 * 3,
                        () =>
                            `§fill=${menuFinder('mobileSettings', 'multiconfig') ? '#ff0' : '#fff'}§${!SAVE.flag.b.$option_mobileEnableMultiConfig ? '\u4dc4' : '\u4dc5'
                            }`
                    ),
                    menuText(40, 84 + 35 * 4 + 10, () => `§fill=#0ff§${text.menu.mobile.prompt2}`, {
                        fontFamily: content.fDiaryOfAn8BitMage,
                        fontSize: 10
                    }),
                    menuText(
                        ...
                        84 + 35 * 5,
                        () =>
                            `§fill=${menuFinder('mobileSettings', 'type') ? '#ff0' : '#fff'}§${text.menu.mobile.controlType}`
                    ),
                    menuText(360, 84 + 35 * 5, () => {
                        const selected = menuFinder('mobileSettings', 'type');
                        return `${selected ? '§fill=#ff0§< ' : ''}§fill=${selected ? '#ff0' : '#fff'}§${SAVE.flag.n.$option_mobileControlType === 0 ? '\u4dc6' : '\u4dc7'
                            }${selected ? '§fill=#ff0§ >' : ''}`;
                    }).on('pre-render', function () {
                        this.offsets[0].x = menuFinder('mobileSettings', 'type')
                            ? -content.fDeterminationSans.metrics('< ').x
                            : 0;
                    }),
                    menuText(
                        ...
                        84 + 35 * 6,
                        () =>
                            `§fill=${menuFinder('mobileSettings', 'opacity') ? '#ff0' : '#fff'}§${text.menu.mobile.controlOpacity
                            }`
                    ),
                    menuText(360, 84 + 35 * 6, () => {
                        const selected = menuFinder('mobileSettings', 'opacity');
                        return `${selected ? `§fill=${SAVE.flag.n.$option_mobileOpacity === 0 ? '#808000' : '#ff0'}§< ` : ''
                            }§fill=${selected ? '#ff0' : '#fff'}§${text.general.percent.replace(
                                '$(x)',
                                Math.round(SAVE.flag.n.$option_mobileOpacity * 100).toString()
                            )}${selected ? `§fill=${SAVE.flag.n.$option_mobileOpacity === 1 ? '#808000' : '#ff0'}§ >` : ''}`;
                    }).on('pre-render', function () {
                        this.offsets[0].x = menuFinder('mobileSettings', 'opacity')
                            ? -content.fDeterminationSans.metrics('< ').x
                            : 0;
                    }),
                    menuText(
                        ...
                        84 + 35 * 7,
                        () =>
                            `§fill=${menuFinder('mobileSettings', 'invertbutton') ? '#ff0' : '#fff'}§${text.menu.mobile.invertButtonPos
                            }`
                    ),
                    ...
                    menuText(
                        ...
                        84 + 35 * 8.5,
                        () =>
                            `§fill=${menuFinder('mobileSettings', 'nextpage') ? '#ff0' : '#fff'}§${text.menu.mobile.nextpage}`
                    ),
                    menuText(
                        40,
                        84 + 35 * 9.5,
                        () =>
                            `${menuFinder('mobileSettings', 'loaddefault')
                                ? text.menu.mobile.helper_loadDefault
                                : menuFinder('mobileSettings', 'multiconfig')
                                    ? text.menu.mobile.helper_enableMultiConfig
                                    : menuFinder('mobileSettings', 'type')
                                        ? text.menu.mobile.helper_controlType
                                        : menuFinder('mobileSettings', 'opacity')
                                            ? text.menu.mobile.helper_controlOpacity
                                            : menuFinder('mobileSettings', 'invertbutton')
                                                ? text.menu.mobile.helper_invertButtonPos
                                                : ''
                            }`,
                        {
                            fontFamily: content.fDeterminationSans,
        ...
        .on('from', function () {
            ...
            atlas.attach(renderer, 'menu', 'mobileSettings');
        })
        .on('to', function () {
            atlas.detach(renderer, 'menu', 'mobileSettings');
        }),
    mobileSettings2: new CosmosNavigator<string>({
        flip: true,
        grid: () => [
            ['showfullscrkey'],
            ...(SAVE.flag.n.$option_mobileControlType === 0
                ? [['mobiledeadzone'], ['mobilearc']]
                : [['diagnal'], ['snglarrow']]),
            ['z', 'x', 'c'],
            SAVE.flag.n.$option_mobileControlType === 0
                ? ['joystick']
                : SAVE.flag.b.$option_mobileEnableSingleArrow
                    ? SAVE.flag.b.$option_mobileEnableDiagonal
                        ? ['l', 'u', 'r', 'd', 'lu', 'ru', 'rd', 'ld']
                        : ['l', 'u', 'r', 'd']
                    : SAVE.flag.b.$option_mobileEnableDiagonal
                        ? ['arrowkeys', 'diagarrowkeys']
                        : ['arrowkeys'],
            ['prepage']
        ],
        prev() {
        ...
        next(self) {
            switch (self.selection()) {
                case 'diagnal':
                    SAVE.flag.b.$option_mobileEnableDiagonal = !SAVE.flag.b.$option_mobileEnableDiagonal;
                    frontEnder.updateDiagonalButton();
                    break;
                case 'showfullscrkey':
                    SAVE.flag.b.$option_mobileShowFullScrKey = !SAVE.flag.b.$option_mobileShowFullScrKey;
                    frontEnder.updateShowFullScrKey();
                    break;
                case 'snglarrow':
                    SAVE.flag.b.$option_mobileEnableSingleArrow = !SAVE.flag.b.$option_mobileEnableSingleArrow;
                    break;
                case 'prepage':
                    atlas.switch('mobileSettings');
                    break;
                ...
                case 'ld':
                    ...
                    const Keyassets = {
                        z: mobileAssets.ieButtonZ,
                        x: mobileAssets.ieButtonX,
                        c: mobileAssets.ieButtonC,
                        l: mobileAssets.ieButtonR,
                        u: mobileAssets.ieButtonR,
                        r: mobileAssets.ieButtonR,
                        d: mobileAssets.ieButtonR,
                        lu: mobileAssets.ieButtonTR,
                        ru: mobileAssets.ieButtonTR,
                        rd: mobileAssets.ieButtonTR,
                        ld: mobileAssets.ieButtonTR
                    };
                    const KeyName = {
                        z: text.menu.mobile.ZKey,
                        x: text.menu.mobile.XKey,
                        c: text.menu.mobile.CKey,
                        l: text.menu.mobile.LKey,
                        u: text.menu.mobile.UKey,
                        r: text.menu.mobile.RKey,
                        d: text.menu.mobile.DKey,
                        lu: text.menu.mobile.LUKey,
                        ru: text.menu.mobile.RUKey,
                        rd: text.menu.mobile.RDKey,
                        ld: text.menu.mobile.LDKey
                    };
                    mobileBtnModify.name = KeyName[self.selection() as KeymapKey];
                    mobileBtnModify.key = Keymap[self.selection() as KeymapKey];
                    mobileBtnModify.source = Keyassets[self.selection() as KeymapKey];
                    mobileBtnModify.rotation = mobileBtnModify.key.rotation;

                    mobileBtnModify.step = 1;
                    mobileBtnModify.x = mobileBtnModify.key.x;
                    mobileBtnModify.y = mobileBtnModify.key.y;
                    mobileBtnModify.relative = mobileBtnModify.key.relative;
                    mobileBtnModify.size = mobileBtnModify.key.size;

                    content.asPurchase.loaded && sounds.purchase.instance(renderer);
                    mobileTempBtn1.forEach(btn => {
                        btn.element.style.display = 'block';
                        btn.element.style.backgroundColor = '#0bf';
                        btn.element.style.backgroundBlendMode = 'screen';
                    });
                    mobileTempBtn4.push(
                        new mobileBtn({
                            group: 'temp-keys',
                            source: mobileBtnModify.source,
                            size: mobileBtnModify.size,
                            rotation: mobileBtnModify.rotation,
                            x: mobileBtnModify.x,
                            y: mobileBtnModify.y,
                            relative: mobileBtnModify.relative,
                            onMove(e?: TouchEvent) {
                                if (!e) {
                                    return;
                                }
                                ...
                                let offsetX = 0;
                                let offsetY = 0;
                                if (mobileBtnModify.relative) {
                                    offsetX = (touch.clientX / Math.min(window.innerHeight, window.innerWidth)) * 100;
                                    offsetY = (touch.clientY / Math.min(window.innerHeight, window.innerWidth)) * 100;
                                } else {
                                    offsetX = (touch.clientX / window.innerWidth) * 100;
                                    offsetY = (touch.clientY / window.innerHeight) * 100;
                                }
                                if (mobileTempBtn4[0]) {
                                    mobileTempBtn4[0].setPosAbsolute(offsetX, offsetY);
                                }
                            }
                        })
                    );
                    mobileTempBtn4[0].element.style.backgroundColor = '#fb0';
                    mobileTempBtn4[0].element.style.backgroundBlendMode = 'screen';
                    atlas.switch('mobileSettings3');
                    break;

                case 'joystick':
                case 'arrowkeys':
                case 'diagarrowkeys':
                    atlas.switch('mobileSettings4');
                    break;
            }
        },
        objects: [
            ...
            new CosmosObject({
                ...
                objects: [
                    ...
                    menuText(40, 84 + 10, () => `§fill=#0ff§${text.menu.mobile.prompt3}`, {
                        fontFamily: content.fDiaryOfAn8BitMage,
                        fontSize: 10
                    }),
                    ...
                    menuText(
                        ...
                        84 + 35 * 1,
                        () =>
                            `§fill=${menuFinder('mobileSettings2', 'showfullscrkey') ? '#ff0' : '#fff'}§${text.menu.mobile.ShowFullScrKey
                            }`
                    ),
                    menuText(
                        360,
                        84 + 35 * 1,
                        () =>
                            `§fill=${menuFinder('mobileSettings2', 'showfullscrkey') ? '#ff0' : '#fff'}§${!SAVE.flag.b.$option_mobileShowFullScrKey ? '\u4dc4' : '\u4dc5'
                            }`
                    ),
                    menuText(
                        40,
                        84 + 35 * 2,
                        () =>
                            `§fill=${menuFinder('mobileSettings2', 'mobiledeadzone') ? '#ff0' : '#fff'}§${text.menu.mobile.deadZone
                            }`
                    ).on('tick', function () {
                        this.alpha.value = SAVE.flag.n.$option_mobileControlType === 0 ? 1 : 0;
                    }),
                    menuText(360, 84 + 35 * 2, () => {
                        const selected = menuFinder('mobileSettings2', 'mobiledeadzone');
                        return `${selected ? `§fill=${(mobileJoystk0 as mobileJoystk).deadzone === 0 ? '#808000' : '#ff0'}§< ` : ''
                            }§fill=${selected ? '#ff0' : '#fff'}§${text.general.percent.replace(
                                '$(x)',
                                Math.round((mobileJoystk0 as mobileJoystk).deadzone * 100).toString()
                            )}${selected ? `§fill=${(mobileJoystk0 as mobileJoystk).deadzone === 0.9 ? '#808000' : '#ff0'}§ >` : ''
                            }`;
                    })
                        .on('tick', function () {
                            this.alpha.value = SAVE.flag.n.$option_mobileControlType === 0 ? 1 : 0;
                        ...
                        .on('pre-render', function () {
                            this.offsets[0].x = menuFinder('mobileSettings2', 'mobiledeadzone')
                                ? -content.fDeterminationSans.metrics('< ').x
                                : 0;
                        }),
                    menuText(
                        ...
                        84 + 35 * 3,
                        () =>
                            `§fill=${menuFinder('mobileSettings2', 'mobilearc') ? '#ff0' : '#fff'}§${text.menu.mobile.toleranceAngle
                            }`
                    ).on('tick', function () {
                        this.alpha.value = SAVE.flag.n.$option_mobileControlType === 0 ? 1 : 0;
                    }),

                    menuText(360, 84 + 35 * 3, () => {
                        const selected = menuFinder('mobileSettings2', 'mobilearc');
                        return `${selected ? `§fill=${(mobileJoystk0 as mobileJoystk).angle === 40 ? '#808000' : '#ff0'}§< ` : ''
                            }§fill=${selected ? '#ff0' : '#fff'}§${(mobileJoystk0 as mobileJoystk).angle.toString()}°${selected ? `§fill=${(mobileJoystk0 as mobileJoystk).angle === 85 ? '#808000' : '#ff0'}§ >` : ''
                            }`;
                    })
                        .on('tick', function () {
                            this.alpha.value = SAVE.flag.n.$option_mobileControlType === 0 ? 1 : 0;
                        ...
                        .on('pre-render', function () {
                            this.offsets[0].x = menuFinder('mobileSettings2', 'mobilearc')
                                ? -content.fDeterminationSans.metrics('< ').x
                                : 0;
                        }),
                    ...
                    menuText(
                        ...
                        84 + 35 * 2,
                        () =>
                            `§fill=${menuFinder('mobileSettings2', 'diagnal') ? '#ff0' : '#fff'}§${text.menu.mobile.enableDiagonal
                            }`
                    ).on('tick', function () {
                        this.alpha.value = SAVE.flag.n.$option_mobileControlType === 1 ? 1 : 0;
                    }),
                    menuText(360, 84 + 35 * 2, () => {
                        const selected = menuFinder('mobileSettings2', 'diagnal');
                        return `${selected ? '§fill=#ff0§< ' : ''}§fill=${selected ? '#ff0' : '#fff'}§${SAVE.flag.b.$option_mobileEnableDiagonal ? text.menu.mobile.eightKey : text.menu.mobile.fourKey
                            }${selected ? '§fill=#ff0§ >' : ''}`;
                    })
                        .on('tick', function () {
                            this.alpha.value = SAVE.flag.n.$option_mobileControlType === 1 ? 1 : 0;
                        ...
                        .on('pre-render', function () {
                            this.offsets[0].x = menuFinder('mobileSettings2', 'diagnal')
                                ? -content.fDeterminationSans.metrics('< ').x
                                : 0;
                        }),
                    ...
                    menuText(
                        ...
                        84 + 35 * 3,
                        () =>
                            `§fill=${menuFinder('mobileSettings2', 'snglarrow') ? '#ff0' : '#fff'}§${text.menu.mobile.enableSingleArrow
                            }`
                    ).on('tick', function () {
                        this.alpha.value = SAVE.flag.n.$option_mobileControlType === 1 ? 1 : 0;
                    }),
                    menuText(
                        ...
                        84 + 35 * 3,
                        () =>
                            `§fill=${menuFinder('mobileSettings2', 'snglarrow') ? '#ff0' : '#fff'}§${SAVE.flag.b.$option_mobileEnableSingleArrow ? text.general.enabled : text.general.disabled
                            }`
                    ).on('tick', function () {
                        this.alpha.value = SAVE.flag.n.$option_mobileControlType === 1 ? 1 : 0;
                    }),
                    menuText(40, 84 + 35 * 4 + 10, () => `§fill=#0ff§${text.menu.mobile.prompt4}`, {
                        fontFamily: content.fDiaryOfAn8BitMage,
                        fontSize: 10
                    }),
                    menuText(
                        40,
                        84 + 35 * 5,
                        () => `§fill=${menuFinder('mobileSettings2', 'z') ? '#ff0' : '#fff'}§${text.menu.mobile.ZKey}`
                    ),
                    menuText(
                        200,
                        84 + 35 * 5,
                        () => `§fill=${menuFinder('mobileSettings2', 'x') ? '#ff0' : '#fff'}§${text.menu.mobile.XKey}`
                    ),
                    menuText(
                        360,
                        84 + 35 * 5,
                        () => `§fill=${menuFinder('mobileSettings2', 'c') ? '#ff0' : '#fff'}§${text.menu.mobile.CKey}`
                    ),

                    menuText(
                        40,
                        84 + 35 * 6,
                        () => `§fill=${menuFinder('mobileSettings2', 'joystick') ? '#ff0' : '#fff'}§[\u4dc6]`
                    ).on('tick', function () {
                        this.alpha.value = SAVE.flag.n.$option_mobileControlType === 0 ? 1 : 0;
                    }),
                    ...
                    menuText(
                        ...
                        84 + 35 * 6,
                        () =>
                            `§fill=${menuFinder('mobileSettings2', 'arrowkeys') ? '#ff0' : '#fff'}§${text.menu.mobile.arrowKeys
                            }`
                    ).on('tick', function () {
                        this.alpha.value =
                            SAVE.flag.n.$option_mobileControlType === 1 && !SAVE.flag.b.$option_mobileEnableSingleArrow
                                ? 1
                                : 0;
                    }),
                    menuText(
                        ...
                        84 + 35 * 6,
                        () =>
                            `§fill=${menuFinder('mobileSettings2', 'diagarrowkeys') ? '#ff0' : '#fff'}§${text.menu.mobile.diagarrowKeys
                            }`
                    ).on('tick', function () {
                        this.alpha.value =
                            SAVE.flag.n.$option_mobileControlType === 1 &&
                                !SAVE.flag.b.$option_mobileEnableSingleArrow &&
                                SAVE.flag.b.$option_mobileEnableDiagonal
                                ? 1
                                : 0;
                    ...
                    menuText(
                        40,
                        84 + 35 * 6,
                        () => `§fill=${menuFinder('mobileSettings2', 'l') ? '#ff0' : '#fff'}§${text.menu.mobile.LKey}`
                    ).on('tick', function () {
                        this.alpha.value =
                            SAVE.flag.n.$option_mobileControlType === 1 && SAVE.flag.b.$option_mobileEnableSingleArrow ? 1 : 0;
                    }),
                    menuText(
                        90,
                        84 + 35 * 6,
                        () => `§fill=${menuFinder('mobileSettings2', 'u') ? '#ff0' : '#fff'}§${text.menu.mobile.UKey}`
                    ).on('tick', function () {
                        this.alpha.value =
                            SAVE.flag.n.$option_mobileControlType === 1 && SAVE.flag.b.$option_mobileEnableSingleArrow ? 1 : 0;
                    }),
                    menuText(
                        140,
                        84 + 35 * 6,
                        () => `§fill=${menuFinder('mobileSettings2', 'r') ? '#ff0' : '#fff'}§${text.menu.mobile.RKey}`
                    ).on('tick', function () {
                        this.alpha.value =
                            SAVE.flag.n.$option_mobileControlType === 1 && SAVE.flag.b.$option_mobileEnableSingleArrow ? 1 : 0;
                    }),
                    menuText(
                        190,
                        84 + 35 * 6,
                        () => `§fill=${menuFinder('mobileSettings2', 'd') ? '#ff0' : '#fff'}§${text.menu.mobile.DKey}`
                    ).on('tick', function () {
                        this.alpha.value =
                            SAVE.flag.n.$option_mobileControlType === 1 && SAVE.flag.b.$option_mobileEnableSingleArrow ? 1 : 0;
                    }),
                    menuText(
                        270,
                        84 + 35 * 6,
                        () => `§fill=${menuFinder('mobileSettings2', 'lu') ? '#ff0' : '#fff'}§${text.menu.mobile.LUKey}`
                    ).on('tick', function () {
                        this.alpha.value =
                            SAVE.flag.n.$option_mobileControlType === 1 &&
                                SAVE.flag.b.$option_mobileEnableDiagonal &&
                    ...
                    menuText(
                        320,
                        84 + 35 * 6,
                        () => `§fill=${menuFinder('mobileSettings2', 'ru') ? '#ff0' : '#fff'}§${text.menu.mobile.RUKey}`
                    ).on('tick', function () {
                        this.alpha.value =
                            SAVE.flag.n.$option_mobileControlType === 1 &&
                                SAVE.flag.b.$option_mobileEnableDiagonal &&
                    ...
                    menuText(
                        370,
                        84 + 35 * 6,
                        () => `§fill=${menuFinder('mobileSettings2', 'rd') ? '#ff0' : '#fff'}§${text.menu.mobile.RDKey}`
                    ).on('tick', function () {
                        this.alpha.value =
                            SAVE.flag.n.$option_mobileControlType === 1 &&
                                SAVE.flag.b.$option_mobileEnableDiagonal &&
                    ...
                    menuText(
                        420,
                        84 + 35 * 6,
                        () => `§fill=${menuFinder('mobileSettings2', 'ld') ? '#ff0' : '#fff'}§${text.menu.mobile.LDKey}`
                    ).on('tick', function () {
                        this.alpha.value =
                            SAVE.flag.n.$option_mobileControlType === 1 &&
                                SAVE.flag.b.$option_mobileEnableDiagonal &&
                    ...
                    menuText(
                        ...
                        84 + 35 * 8.5,
                        () =>
                            `§fill=${menuFinder('mobileSettings2', 'prepage') ? '#ff0' : '#fff'}§${text.menu.mobile.prepage}`
                    ),
                    ...
                    menuText(
                        ...
                        84 + 35 * 9.5,
                        () =>
                            `${menuFinder('mobileSettings2', 'showfullscrkey')
                                ? text.menu.mobile.helper_showFullScrKey
                                : menuFinder('mobileSettings2', 'diagnal')
                                    ? text.menu.mobile.helper_enableDiagnal
                                    : menuFinder('mobileSettings2', 'mobiledeadzone')
                                        ? text.menu.mobile.helper_deadZone
                                        : menuFinder('mobileSettings2', 'mobilearc')
                                            ? text.menu.mobile.helper_toleranceAngle
                                            : menuFinder('mobileSettings2', 'snglarrow')
                                                ? text.menu.mobile.helper_singleArrow
                                                : menuFinder('mobileSettings2', 'propage')
                                                    ? ''
                                                    : text.menu.mobile.helper_modifyButtons
                            }`,
                        {
                            fontFamily: content.fDeterminationSans,
                            fontSize: 8,
        ...
        .on('from', function () {
            this.position = { x: 0, y: 0 };
            atlas.attach(renderer, 'menu', 'mobileSettings2');
        })
        .on('to', function () {
            atlas.detach(renderer, 'menu', 'mobileSettings2');
        }),
    mobileSettings3: new CosmosNavigator<string>({
        grid: () => [['confirm']],
        prev() {
            switch (mobileBtnModify.step) {
                case 1:
                    mobileTempBtn4[0].setPosAbsolute(mobileBtnModify.x, mobileBtnModify.y);
                    break;
                case 2:
                    mobileTempBtn4[0].setPosAbsolute(mobileBtnModify.x, mobileBtnModify.y);
                    break;
                case 3:
                    mobileTempBtn4[0].resize(mobileBtnModify.size);
                    break;
                case 0:
                case 4:
                    [...mobileTempBtn1, ...mobileTempBtn2, ...mobileTempBtn3, ...mobileTempBtn4].forEach(btn => {
                        btn.element.style.display = 'none';
                    });
                    mobileTempBtn4.pop();
                    mobileEnableBtninput();
        ...
        next(self) {
            const currentTempBtn = mobileTempBtn4[0];
            switch (mobileBtnModify.step) {
                case 0:
                    // 创建临时按钮
                    mobileBtnModify.step = 1;
                ...
                case 1:
                    ...
                    mobileBtnModify.x = Math.round(currentTempBtn.x);
                    mobileBtnModify.y = Math.round(currentTempBtn.y);

                    currentTempBtn.onMove = () => { };
                    currentTempBtn.element.style.pointerEvents = 'none';
                    mobileTempBtn2.forEach(btn => {
                        btn.element.style.display = 'block';
                        btn.element.style.backgroundColor = '#0bf';
                        btn.element.style.backgroundBlendMode = 'screen';
                    });
                    mobileBtnModify.step = 2;
                    break;
                case 2:
                    // 写入临时设置
                    mobileBtnModify.x = mobileTempBtn4[0].x;
                    mobileBtnModify.y = mobileTempBtn4[0].y;
                    // 调节大小
                    // 关闭临时方向键，启动数字键
                    mobileTempBtn2.forEach(btn => {
                        btn.element.style.display = 'none';
                    });
                    mobileTempBtn3.forEach(btn => {
                        btn.element.style.display = 'block';
                        btn.element.style.backgroundColor = '#0bf';
                        btn.element.style.backgroundBlendMode = 'screen';
                    });
                    mobileBtnModify.step = 3;
                    break;
                case 3:
                    // 写入临时设置
                    mobileBtnModify.size = mobileTempBtn4[0].size;
                    mobileTempBtn3.forEach(btn => {
                        btn.element.style.display = 'none';
                    });
                    mobileBtnModify.step = 4;
                    break;
                case 4:
                    // 将设置写入到对应按键
                    if (mobileBtnModify.key) {
                        mobileBtnModify.key.setPosAbsolute(mobileBtnModify.x, mobileBtnModify.y);
                        mobileBtnModify.key.resize(mobileBtnModify.size);
                    }
                    // 恢复原来的按键
                    [...mobileTempBtn1, ...mobileTempBtn2, ...mobileTempBtn3, ...mobileTempBtn4].forEach(btn => {
                        btn.element.style.display = 'none';
        ...
        objects: [
            ...
            new CosmosObject({
                ...
                priority: Number.MAX_SAFE_INTEGER,
                objects: [
                    menuText(320, 36 - 4, () => text.menu.mobile.keysettings, {
                        fontFamily: content.fDeterminationSans,
                        fontSize: 32
                    }).on('tick', function () {
                        this.offsets[0].x = -values.textLengthPrecise(this.content) * 7;
                    ...
                    menuText(40, 84 + 30, () => `${text.menu.mobile.keyprompt0.replace('$(x)', mobileBtnModify.name)}`).on(
                        'tick',
                        function () {
                            this.alpha.value = SAVE.flag.n.$option_mobileControlType === 0 ? 1 : 0;
                        }
                    ...
                    menuText(
                        40,
                        84 + 35 * 2,
                        () => `${text.menu.mobile.keyprompt1.replace('$(x)', Math.min(3, mobileBtnModify.step).toString())}`
                    ).on('tick', function () {
                        this.alpha.value = mobileBtnModify.step >= 1 ? 1 : 0;
                    }),
                    menuText(40, 84 + 35 * 3, () => `${text.menu.mobile.keyprompt1d}`).on('tick', function () {
                        this.alpha.value = mobileBtnModify.step === 0 ? 1 : 0;
                    }),
                    menuText(40, 84 + 35 * 3, () => `${text.menu.mobile.keyprompt1e}`).on('tick', function () {
                        this.alpha.value = mobileBtnModify.step === 4 ? 1 : 0;
                    }),
                    menuText(
                        40,
                        84 + 35 * 3,
                        () =>
                            `${mobileBtnModify.step === 1 ? '->§fill=#ffd700§' : '  §fill=#808080§'}${text.menu.mobile.keyprompt1a
                            }`
                    ).on('tick', function () {
                        this.alpha.value = mobileBtnModify.step >= 1 && mobileBtnModify.step < 4 ? 1 : 0;
                    }),
                    menuText(
                        40,
                        84 + 35 * 4,
                        () =>
                            `${mobileBtnModify.step === 2 ? '->§fill=#ffd700§' : '  §fill=#808080§'}${text.menu.mobile.keyprompt1b
                            }`
                    ).on('tick', function () {
                        this.alpha.value = mobileBtnModify.step >= 2 && mobileBtnModify.step < 4 ? 1 : 0;
                    }),
                    menuText(
                        40,
                        84 + 35 * 5,
                        () =>
                            `${mobileBtnModify.step === 3 ? '->§fill=#ffd700§' : '  §fill=#808080§'}${text.menu.mobile.keyprompt1c
                            }`
                    ).on('tick', function () {
                        this.alpha.value = mobileBtnModify.step >= 3 && mobileBtnModify.step < 4 ? 1 : 0;
                    }),
                    menuText(40, 84 + 35 * 6, () => `${text.menu.mobile.keyprompt2}`, {
                        fontFamily: content.fDeterminationSans,
                        fontSize: 16
                    }),
                    menuText(
                        40,
                        84 + 35 * 7,
                        () => `${text.menu.mobile.keyprompt2a.replace('$(x)', mobileBtnModify.name)}`
                    ).on('tick', function () {
                        this.alpha.value = mobileBtnModify.step === 1 ? 1 : 0;
                    }),
                    menuText(
                        40,
                        84 + 35 * 8,
                        () => `${text.menu.mobile.keyprompt2b.replace('$(x)', mobileBtnModify.name)}`
                    ).on('tick', function () {
                        this.alpha.value = mobileBtnModify.step === 2 ? 1 : 0;
                    }),
                    menuText(
                        40,
                        84 + 35 * 8,
                        () => `${text.menu.mobile.keyprompt2c.replace('$(x)', mobileBtnModify.name)}`
                    ).on('tick', function () {
                        this.alpha.value = mobileBtnModify.step === 3 ? 1 : 0;
                    }),
                    ...
                    menuText(
                        ...
                        84 + 35 * 2,
                        () =>
                            `${text.menu.mobile.keyprompt3a
                                .replace('$(x)', (Math.round(mobileTempBtn4[0].x * 10) / 10).toString())
                                .replace('$(y)', (Math.round(mobileTempBtn4[0].y * 10) / 10).toString())}`,
                        { fontFamily: content.fDeterminationSans, fontSize: 16 }
                    ),
                    menuText(
                        450,
                        84 + 35 * 4,
                        () => `${text.menu.mobile.keyprompt3b.replace('$(x)', mobileTempBtn4[0].size.toString())}`,
                        { fontFamily: content.fDeterminationSans, fontSize: 16 }
                    )
                ]
            })
        ...
        .on('from', function () {
            this.position = { x: 0, y: 0 };
            atlas.attach(renderer, 'menu', 'mobileSettings3');
            mobileDisableBtninput();
        })
        .on('to', function () {
            atlas.detach(renderer, 'menu', 'mobileSettings3');
        }),
    mobileSettings4: new CosmosNavigator<string>({
        grid: () => [
            [
                'exit',
                ...(SAVE.flag.n.$option_mobileControlType === 0
                    ? ['joystickx', 'joysticky', 'joysticksize']
                    : SAVE.flag.b.$option_mobileEnableDiagonal
                        ? ['arrowx', 'arrowy', 'arrowsize', 'arrowradius', 'diagsize', 'diagradius']
                        : ['arrowx', 'arrowy', 'arrowsize', 'arrowradius'])
            ]
        ],
        next: self => {
            const selection = self.selection() as string;
            ...
            new CosmosObject({
                ...
                objects: [
                    ...
                    menuText(
                        40,
                        84 + 5,
                        () => `§fill=${menuFinder('mobileSettings4', 'exit') ? '#ff0' : '#fff'}§${text.menu.settings2}`
                    ),
                    menuText(40, 84 + 35, () => '§fill=#0ff§-> \u4dc6').on('tick', function () {
                        this.alpha.value = SAVE.flag.n.$option_mobileControlType === 0 ? 1 : 0;
                    }),
                    menuText(
                        40,
                        84 + 35 * 2,
                        () => `§fill=${menuFinder('mobileSettings4', 'joystickx') ? '#ff0' : '#fff'}§${text.menu.mobile.xPos}`
                    ).on('tick', function () {
                        this.alpha.value = SAVE.flag.n.$option_mobileControlType === 0 ? 1 : 0;
                    }),
                    menuText(360, 84 + 35 * 2, () => {
                        const selected = menuFinder('mobileSettings4', 'joystickx');
                        return `${selected ? '§fill=#ff0§< ' : ''}§fill=${selected ? '#ff0' : '#fff'}§${(mobileJoystk0 as mobileJoystk).x
                            }${selected ? '§fill=#ff0§ >' : ''}`;
                    })
                        .on('tick', function () {
                            this.alpha.value = SAVE.flag.n.$option_mobileControlType === 0 ? 1 : 0;
                        ...
                        .on('pre-render', function () {
                            this.offsets[0].x = menuFinder('mobileSettings4', 'joystickx')
                                ? -content.fDeterminationSans.metrics('< ').x
                                : 0;
                        }),
                    ...
                    menuText(
                        40,
                        84 + 35 * 3,
                        () => `§fill=${menuFinder('mobileSettings4', 'joysticky') ? '#ff0' : '#fff'}§${text.menu.mobile.yPos}`
                    ).on('tick', function () {
                        this.alpha.value = SAVE.flag.n.$option_mobileControlType === 0 ? 1 : 0;
                    }),
                    menuText(360, 84 + 35 * 3, () => {
                        const selected = menuFinder('mobileSettings4', 'joysticky');
                        return `${selected ? '§fill=#ff0§< ' : ''}§fill=${selected ? '#ff0' : '#fff'}§${(mobileJoystk0 as mobileJoystk).y
                            }${selected ? '§fill=#ff0§ >' : ''}`;
                    })
                        .on('tick', function () {
                            this.alpha.value = SAVE.flag.n.$option_mobileControlType === 0 ? 1 : 0;
                        ...
                        .on('pre-render', function () {
                            this.offsets[0].x = menuFinder('mobileSettings4', 'joysticky')
                                ? -content.fDeterminationSans.metrics('< ').x
                                : 0;
                        }),
                    ...
                    menuText(
                        ...
                        84 + 35 * 4,
                        () =>
                            `§fill=${menuFinder('mobileSettings4', 'joysticksize') ? '#ff0' : '#fff'}§${text.menu.mobile.radius
                            }`
                    ).on('tick', function () {
                        this.alpha.value = SAVE.flag.n.$option_mobileControlType === 0 ? 1 : 0;
                    }),
                    menuText(360, 84 + 35 * 4, () => {
                        const selected = menuFinder('mobileSettings4', 'joysticksize');
                        return `${selected ? '§fill=#ff0§< ' : ''}§fill=${selected ? '#ff0' : '#fff'}§${(mobileJoystk0 as mobileJoystk).radiusRel
                            }${selected ? '§fill=#ff0§ >' : ''}`;
                    })
                        .on('tick', function () {
                            this.alpha.value = SAVE.flag.n.$option_mobileControlType === 0 ? 1 : 0;
                        ...
                        .on('pre-render', function () {
                            this.offsets[0].x = menuFinder('mobileSettings4', 'joysticksize')
                                ? -content.fDeterminationSans.metrics('< ').x
                                : 0;
                        }),

                    menuText(40, 84 + 35, () => `§fill=#0ff§-> ${text.menu.mobile.arrowKeys}`).on('tick', function () {
                        this.alpha.value = SAVE.flag.n.$option_mobileControlType === 1 ? 1 : 0;
                    }),
                    menuText(
                        40,
                        84 + 35 * 2,
                        () => `§fill=${menuFinder('mobileSettings4', 'arrowx') ? '#ff0' : '#fff'}§${text.menu.mobile.xPos}`
                    ).on('tick', function () {
                        this.alpha.value = SAVE.flag.n.$option_mobileControlType === 1 ? 1 : 0;
                    }),
                    menuText(360, 84 + 35 * 2, () => {
                        const selected = menuFinder('mobileSettings4', 'arrowx');
                        return `${selected ? '§fill=#ff0§< ' : ''}§fill=${selected ? '#ff0' : '#fff'}§${mobileArrowGroup.x}${selected ? '§fill=#ff0§ >' : ''
                            }`;
                    })
                        .on('tick', function () {
                            this.alpha.value = SAVE.flag.n.$option_mobileControlType === 1 ? 1 : 0;
                        ...
                        .on('pre-render', function () {
                            this.offsets[0].x = menuFinder('mobileSettings4', 'arrowx')
                                ? -content.fDeterminationSans.metrics('< ').x
                                : 0;
                        }),
                    ...
                    menuText(
                        40,
                        84 + 35 * 3,
                        () => `§fill=${menuFinder('mobileSettings4', 'arrowy') ? '#ff0' : '#fff'}§${text.menu.mobile.yPos}`
                    ).on('tick', function () {
                        this.alpha.value = SAVE.flag.n.$option_mobileControlType === 1 ? 1 : 0;
                    }),
                    menuText(360, 84 + 35 * 3, () => {
                        const selected = menuFinder('mobileSettings4', 'arrowy');
                        return `${selected ? '§fill=#ff0§< ' : ''}§fill=${selected ? '#ff0' : '#fff'}§${mobileArrowGroup.y}${selected ? '§fill=#ff0§ >' : ''
                            }`;
                    })
                        .on('tick', function () {
                            this.alpha.value = SAVE.flag.n.$option_mobileControlType === 1 ? 1 : 0;
                        ...
                        .on('pre-render', function () {
                            this.offsets[0].x = menuFinder('mobileSettings4', 'arrowy')
                                ? -content.fDeterminationSans.metrics('< ').x
                                : 0;
                        }),
                    ...
                    menuText(
                        40,
                        84 + 35 * 4,
                        () => `§fill=${menuFinder('mobileSettings4', 'arrowsize') ? '#ff0' : '#fff'}§${text.menu.mobile.size}`
                    ).on('tick', function () {
                        this.alpha.value = SAVE.flag.n.$option_mobileControlType === 1 ? 1 : 0;
                    }),
                    menuText(360, 84 + 35 * 4, () => {
                        const selected = menuFinder('mobileSettings4', 'arrowsize');
                        return `${selected ? '§fill=#ff0§< ' : ''}§fill=${selected ? '#ff0' : '#fff'}§${mobileArrowGroup.arrowsize
                            }${selected ? '§fill=#ff0§ >' : ''}`;
                    })
                        .on('tick', function () {
                            this.alpha.value = SAVE.flag.n.$option_mobileControlType === 1 ? 1 : 0;
                        ...
                        .on('pre-render', function () {
                            this.offsets[0].x = menuFinder('mobileSettings4', 'arrowsize')
                                ? -content.fDeterminationSans.metrics('< ').x
                                : 0;
                        }),
                    ...
                    menuText(
                        ...
                        84 + 35 * 5,
                        () =>
                            `§fill=${menuFinder('mobileSettings4', 'arrowradius') ? '#ff0' : '#fff'}§${text.menu.mobile.radius
                            }`
                    ).on('tick', function () {
                        this.alpha.value = SAVE.flag.n.$option_mobileControlType === 1 ? 1 : 0;
                    }),
                    menuText(360, 84 + 35 * 5, () => {
                        const selected = menuFinder('mobileSettings4', 'arrowradius');
                        return `${selected ? '§fill=#ff0§< ' : ''}§fill=${selected ? '#ff0' : '#fff'}§${mobileArrowGroup.arrowradius
                            }${selected ? '§fill=#ff0§ >' : ''}`;
                    })
                        .on('tick', function () {
                            this.alpha.value = SAVE.flag.n.$option_mobileControlType === 1 ? 1 : 0;
                        ...
                        .on('pre-render', function () {
                            this.offsets[0].x = menuFinder('mobileSettings4', 'arrowradius')
                                ? -content.fDeterminationSans.metrics('< ').x
                                : 0;
                        }),

                    menuText(40, 84 + 35 * 6, () => `§fill=#0ff§-> ${text.menu.mobile.diagarrowKeys}`).on(
                        'tick',
                        function () {
                            this.alpha.value =
                                SAVE.flag.n.$option_mobileControlType === 1 && SAVE.flag.b.$option_mobileEnableDiagonal ? 1 : 0;
                        }
                    ),
                    menuText(
                        40,
                        84 + 35 * 7,
                        () => `§fill=${menuFinder('mobileSettings4', 'diagsize') ? '#ff0' : '#fff'}§${text.menu.mobile.size}`
                    ).on('tick', function () {
                        this.alpha.value =
                            SAVE.flag.n.$option_mobileControlType === 1 && SAVE.flag.b.$option_mobileEnableDiagonal ? 1 : 0;
                    }),
                    menuText(360, 84 + 35 * 7, () => {
                        const selected = menuFinder('mobileSettings4', 'diagsize');
                        return `${selected ? '§fill=#ff0§< ' : ''}§fill=${selected ? '#ff0' : '#fff'}§${mobileArrowGroup.diagsize
                            }${selected ? '§fill=#ff0§ >' : ''}`;
                    })
                        .on('tick', function () {
                            this.alpha.value =
                        ...
                        .on('pre-render', function () {
                            this.offsets[0].x = menuFinder('mobileSettings4', 'diagsize')
                                ? -content.fDeterminationSans.metrics('< ').x
                                : 0;
                        }),
                    ...
                    menuText(
                        ...
                        84 + 35 * 8,
                        () =>
                            `§fill=${menuFinder('mobileSettings4', 'diagradius') ? '#ff0' : '#fff'}§${text.menu.mobile.radius}`
                    ).on('tick', function () {
                        this.alpha.value =
                            SAVE.flag.n.$option_mobileControlType === 1 && SAVE.flag.b.$option_mobileEnableDiagonal ? 1 : 0;
                    }),
                    menuText(360, 84 + 35 * 8, () => {
                        const selected = menuFinder('mobileSettings4', 'diagradius');
                        return `${selected ? '§fill=#ff0§< ' : ''}§fill=${selected ? '#ff0' : '#fff'}§${mobileArrowGroup.diagradius
                            }${selected ? '§fill=#ff0§ >' : ''}`;
                    })
                        .on('tick', function () {
                            this.alpha.value =
                        ...
                        .on('pre-render', function () {
                            this.offsets[0].x = menuFinder('mobileSettings4', 'diagradius')
                                ? -content.fDeterminationSans.metrics('< ').x
                                : 0;
                        })
                ]
        ...
        .on('from', function () {
            this.position = { x: 0, y: 0 };
            atlas.attach(renderer, 'menu', 'mobileSettings4');
        })
        .on('to', function () {
            atlas.detach(renderer, 'menu', 'mobileSettings4');
        }),
            ...
            menuBox(420, 240, 206, 226, 8, {
                ...
                objects: [
                    ...
                    menuText(
                        32,
                        180 - 4,
                        () => `${saver.gold === Infinity ? text.general.inf : saver.gold}${text.general.shopg} `
                    ),
            ...
            menuBox(32, 52, 130, 98, 6, {
                ...
                objects: [
                    menuText(
                        ...
                        () =>
                            SAVE.data.b.svr || SAVE.flag.n.lv20 !== 0
                                ? text.general.frisk
                                : SAVE.data.s.name || text.general.mystery1,
                        { fontFamily: content.fDeterminationMono, fontSize: 16 }
                    ).on('tick', function () {
                        if (SAVE.data.s.name !== '' && values.textLengthPrecise(SAVE.data.s.name) >= 8) {
                            this.fontFamily = content.fCryptOfTomorrow;
                            this.fontSize = 8;
                            this.offsets[0].y = 2;
                        } else {
                            this.fontFamily = content.fDeterminationMono;
                            this.fontSize = 16;
                            this.offsets[0].y = 0;
                        }
                    }),
                    ...
                    menuText(
                        ...
                        60 - 2,
                        () =>
                            `${SAVE.data.n.hp === Infinity ? text.general.inf : SAVE.data.n.hp}${world.archive ? '' : `/${calcHP()}`
                            }`
                    ),
                    menuText(44, 78 - 2, () =>
                        world.archive
            ...
            menuBox(188, 52, 0, 406, 6, {
                ...
                objects: [
                    ...
                    menuText(
                        ...
                        94 - 4,
                        () =>
                            `${world.archive ? text.general.xm : text.general.lv} ${SAVE.data.b._overrideStat ? SAVE.data.s._LV : world.archive ? SAVE.data.n.xm + 1 : calcLV()
                            }`
                    ),
                    menuText(
                        22,
                        126 - 4,
                        () =>
                            `${text.general.hp} ${SAVE.data.n.hp === Infinity ? text.general.inf : SAVE.data.n.hp}${world.archive ? '' : ` / ${calcHP()}`
                            }`
                    ),
                    menuText(
                        22,
                        190 - 4,
                        () =>
                            `${text.menu.stat1} ${calcAT()}${SAVE.data.b._overrideStat
                                ? ''
                    ...
                    menuText(
                        ...
                        222 - 4,
                        () =>
                            `${text.menu.stat2} ${calcDF()}${SAVE.data.b._overrideStat ? '' : `(${items.of(SAVE.data.s.armor).value})`
                            }`
                    ),
...
keys.downKey.on('down', () => {
    ...
    if (game.input && atlas.target === 'mobileSettings3') {
        sounds.menu.instance(renderer);
        switch (mobileBtnModify.step) {
            case 2:
                mobileTempBtn4[0].setPos(0, 0.5);
                break;
            case 3:
                mobileTempBtn4[0].resize(mobileTempBtn4[0].size + 5);
                break;
            default:
                break;
        }
...
keys.leftKey.on('down', () => {
    ...
    keyState.left = true;
    if (game.input && atlas.target === 'mobileSettings') {
        const selection = atlas.navigators.of('mobileSettings').selection() as string;
        switch (selection) {
            case 'opacity':
                sounds.menu.instance(renderer);
                SAVE.flag.n.$option_mobileOpacity =
                    Math.round(Math.max(SAVE.flag.n.$option_mobileOpacity - 0.1, 0) * 100) / 100;
                frontEnder.updatemobileOpacity();
                break;
            case 'type':
                sounds.menu.instance(renderer);
                SAVE.flag.n.$option_mobileControlType = SAVE.flag.n.$option_mobileControlType === 1 ? 0 : 1;
    ...
    if (game.input && atlas.target === 'mobileSettings2') {
        const selection = atlas.navigators.of('mobileSettings2').selection() as string;
        switch (selection) {
            case 'mobiledeadzone':
                sounds.menu.instance(renderer);
                if ((mobileJoystk0 as mobileJoystk).deadzone > 0) {
                    (mobileJoystk0 as mobileJoystk).deadzone = Math.max(0, (mobileJoystk0 as mobileJoystk).deadzone - 0.02);
                    (mobileJoystk0 as mobileJoystk).draw();
                }
                break;
            case 'mobilearc':
                sounds.menu.instance(renderer);
                if ((mobileJoystk0 as mobileJoystk).angle > 45) {
                    (mobileJoystk0 as mobileJoystk).angle -= 1;
                }
                break;
            case 'diagnal':
                sounds.menu.instance(renderer);
    ...
    if (game.input && atlas.target === 'mobileSettings3') {
        ...
        switch (mobileBtnModify.step) {
            case 2:
                if (mobileTempBtn4[0].x > 0) {
                    mobileTempBtn4[0].setPos(-0.5, 0);
                }
                break;
            case 3:
                if (mobileTempBtn4[0].size >= 5) {
                    mobileTempBtn4[0].resize(mobileTempBtn4[0].size - 5);
                }
                break;
            default:
    ...
    if (game.input && atlas.target === 'mobileSettings4') {
        sounds.menu.instance(renderer);
        const selection = atlas.navigators.of('mobileSettings4').selection() as string;
        switch (selection) {
            case 'joystickx':
                if ((mobileJoystk0 as mobileJoystk).x >= 1) {
                    (mobileJoystk0 as mobileJoystk).setPos(-1, 0);
                }
                break;
            case 'joysticky':
                if ((mobileJoystk0 as mobileJoystk).y >= 1) {
                    (mobileJoystk0 as mobileJoystk).setPos(0, -1);
                }
                break;

            case 'joysticksize':
                if ((mobileJoystk0 as mobileJoystk).radiusRel >= 1) {
                    (mobileJoystk0 as mobileJoystk).resize((mobileJoystk0 as mobileJoystk).radiusRel - 1);
                }
                break;

            case 'arrowx':
                if (mobileArrowGroup.x >= 1) {
                    mobileArrowGroup.x -= 1;
                }
                [...mobileArrowBtns, ...mobileDiagBtns].forEach(btn => {
                    btn.setPos(-1, 0);
                });
                break;
            case 'arrowy':
                if (mobileArrowGroup.y >= 1) {
                    mobileArrowGroup.y -= 1;
                }
                [...mobileArrowBtns, ...mobileDiagBtns].forEach(btn => {
                    btn.setPos(0, -1);
                });
                break;
            case 'arrowsize':
                if (mobileArrowGroup.arrowsize >= 1) {
                    mobileArrowGroup.arrowsize -= 1;
                }
                mobileArrowBtns.forEach(btn => {
                    btn.resize(mobileArrowGroup.arrowsize);
                });
                break;
            case 'arrowradius':
                if (mobileArrowGroup.arrowradius >= -99) {
                    mobileArrowGroup.arrowradius -= 1;
                }
                mobileArrowBtns.forEach(btn => {
                    switch (btn.rotation) {
                        case 180: // left
                ...
                break;
            case 'diagsize':
                if (mobileArrowGroup.diagsize > 1) {
                    mobileArrowGroup.diagsize -= 1;
                }
                mobileDiagBtns.forEach(btn => {
                    btn.resize(mobileArrowGroup.diagsize);
                });
                break;
            case 'diagradius':
                if (mobileArrowGroup.diagradius >= -99) {
                    mobileArrowGroup.diagradius -= 1;
                }
                mobileDiagBtns.forEach(btn => {
                    switch (btn.rotation) {
                        case -90: // LU
    ...
    if (game.input && atlas.target === 'frontEndSettings') {
        ...
        if (selection === 'language') {
            ...
            if (!frontEnder.muteMusic && frontEnder.volumeMusic !== 0) {
                SAVE.flag.n.$option_music = Math.round((1 - Math.max(frontEnder.volumeMusic - 0.1, 0)) * 100) / 100;
                frontEnder.testMusic();
            }
        } else if (selection === 'sfx') {
            if (!frontEnder.muteSFX && frontEnder.volumeSFX !== 0) {
                SAVE.flag.n.$option_sfx = Math.round((1 - Math.max(frontEnder.volumeSFX - 0.1, 0)) * 100) / 100;
                frontEnder.testSFX();
            }
        } else if (selection === 'deadzone') {
            if (gamepadder.deadzone !== 0) {
                SAVE.flag.n.$option_deadzone = Math.round(Math.max(gamepadder.deadzone - 0.1, 0) * 100) / 100;
                frontEnder.updateDeadzone();
            }
        } else if (selection === 'border') {
            const borderList = gameBorder.getAvaliableBorder();
            SAVE.flag.n.$option_border = Math.max(0, SAVE.flag.n.$option_border - 1);
            gameBorder.handleUpdateBorder(borderList[SAVE.flag.n.$option_border]);
        }
    }
});
...
keys.rightKey.on('down', () => {
    ...
    if (game.input && atlas.target === 'mobileSettings') {
        ...
        switch (selection) {
            case 'opacity':
                sounds.menu.instance(renderer);
                SAVE.flag.n.$option_mobileOpacity =
                    Math.round(Math.min(SAVE.flag.n.$option_mobileOpacity + 0.1, 1) * 100) / 100;
                frontEnder.updatemobileOpacity();
                break;
            case 'type':
                sounds.menu.instance(renderer);
    ...
    if (game.input && atlas.target === 'mobileSettings2') {
        ...
        switch (selection) {
            case 'mobiledeadzone':
                sounds.menu.instance(renderer);
                if ((mobileJoystk0 as mobileJoystk).deadzone < 0.9) {
                    (mobileJoystk0 as mobileJoystk).deadzone = Math.min(
                        0.9,
                        (mobileJoystk0 as mobileJoystk).deadzone + 0.02
                    );
                    (mobileJoystk0 as mobileJoystk).draw();
                }
                break;
            case 'mobilearc':
                sounds.menu.instance(renderer);
                if ((mobileJoystk0 as mobileJoystk).angle < 85) {
                    (mobileJoystk0 as mobileJoystk).angle += 1;
                }
                break;
            case 'diagnal':
    ...
    if (game.input && atlas.target === 'mobileSettings3') {
        ...
        switch (mobileBtnModify.step) {
            case 2:
                ...
                break;
            case 3:
                mobileTempBtn4[0].resize(mobileTempBtn4[0].size + 1);
                break;
            default:
                break;
        }
    ...
    if (game.input && atlas.target === 'frontEndSettings') {
        ...
        if (selection === 'language') {
            ...
            if (!frontEnder.muteMusic && frontEnder.volumeMusic !== 1) {
                SAVE.flag.n.$option_music = Math.round((1 - Math.min(frontEnder.volumeMusic + 0.1, 1)) * 100) / 100;
                frontEnder.testMusic();
            }
        } else if (selection === 'sfx') {
            if (!frontEnder.muteSFX && frontEnder.volumeSFX !== 1) {
                SAVE.flag.n.$option_sfx = Math.round((1 - Math.min(frontEnder.volumeSFX + 0.1, 1)) * 100) / 100;
                frontEnder.testSFX();
            }
        } else if (selection === 'deadzone') {
            if (gamepadder.deadzone !== 1) {
                SAVE.flag.n.$option_deadzone = Math.round(Math.min(gamepadder.deadzone + 0.1, 1) * 100) / 100;
                frontEnder.updateDeadzone();
            }
        } else if (selection === 'border') {
            const borderList = gameBorder.getAvaliableBorder();
            SAVE.flag.n.$option_border = Math.min(borderList.length - 1, SAVE.flag.n.$option_border + 1);
            gameBorder.handleUpdateBorder(borderList[SAVE.flag.n.$option_border]);
        }
    }
});
...
keys.upKey.on('down', () => {
    ...
    if (game.input && atlas.target === 'mobileSettings3') {
        ...
        switch (mobileBtnModify.step) {
            case 2:
                if (mobileTempBtn4[0].y > 0) {
                    mobileTempBtn4[0].setPos(0, -0.5);
                }
                break;
            case 3:
                if (mobileTempBtn4[0].size >= 1) {
                    mobileTempBtn4[0].resize(mobileTempBtn4[0].size - 1);
                }
                break;
            default:
    ...
    if (delimiter === '/') {
        ...
        switch (key) {
            case 'c':
                ...
                choicer.riseline = +parts[0];
                choicer.options = parts.slice(1);
                choicer.navigator = atlas.target;
                choicer.result = 0;
                choicer.optioncount = parts.slice(1).length;
