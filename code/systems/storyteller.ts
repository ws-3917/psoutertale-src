...
> {
    ...
    next(skip = false) {
        const parseNestedCode = function (line: string, startIndex: number) {
            let balance = 1;
            let index = startIndex;
            while (balance > 0 && index < line.length) {
            ...
            if (balance === 0) {
                return { code: line.slice(startIndex, index - 1), endIndex: index };
            }
            throw new Error(`Unmatched { detected : ${balance}`);
        };
        ...
        while (!done && this.index < line.length) {
            ...
            if (char === '{') {
                this.emit(false);
                const { code, endIndex } = parseNestedCode(line, this.index);
            ...
            if (update) {
                ...
                while (index < this.content.length) {
                    ...
                    if (char === '\n') {
                        ...
                        const dummyCursor =
                            writingMode === 'horizontal-tb' ? { x: cursor.x, y: cursor.y } : { x: cursor.y, y: cursor.x };
                        ...
                        if (glyph !== void 0) {
                            ...
                            const { x, y } =
                                writingMode === 'horizontal-tb' ? { x: dummyX, y: dummyY } : { x: dummyY, y: dummyX };
                            let xOffset = 0;
                            if (writingMode === 'vertical-rl') {
                                ...
                                if (rect instanceof CosmosRectangle) {
                                    xOffset =
                                        ...
                                        this.position.x -
                                        rect.position.x -
                                        (glyph[6] * scale + this.spacing.x + subspacing.x) * stretch;
                                }
                            }
...
> extends CosmosSizedObject<A, B> {
    ...
    visible = false;
    radius: number;
    constructor(options: { r: number; alpha?: number; anchor?: number; stroke?: number; border?: number }) {
        super();
        this.radius = options.r;
        this.alpha.value = options.alpha ?? 1;
        this.anchor.set(options.anchor ?? 0);
    ...
    draw(style: CosmosStyle) {
        ...
        if (((this.radius === 0 || fill === -1) && (this.border === 0 || stroke === -1)) || this.transparent()) {
            if (this.visible) {
                this.visible = false;
                this.container.removeChildAt(0);
            }
            ...
            const origin = new CosmosPoint(this.radius * (1 + this.anchor.x), this.radius * (1 + this.anchor.y));
            graphics.pivot.set(this.radius * 2 * ((this.anchor.x + 1) / 2), this.radius * 2 * ((this.anchor.y + 1) / 2));
            graphics.position.set(origin.x - graphics.pivot.x, origin.y - graphics.pivot.y);
            ...
            graphics.drawCircle(this.radius, this.radius, this.radius).endFill();
...
> extends CosmosSizedObject<A, B> {
    ...
    end_x: number;
    end_y: number;
    constructor(options: { end_x: number; end_y: number; alpha?: number; stroke?: number; border?: number }) {
        super();
        this.end_x = options.end_x;
        this.end_y = options.end_y;
        this.alpha.value = options.alpha ?? 1;
        this.stroke = options.stroke ?? 0x000000;
        this.border = options.border ?? 1;
    }
    draw(style: CosmosStyle) {
        const stroke = this.getStroke(style);
        if (this.border === 0 || stroke === -1 || this.transparent()) {
            if (this.visible) {
                this.visible = false;
                this.container.removeChildAt(0);
            }
            ...
            graphics.moveTo(0, 0).lineTo(this.end_x, this.end_y);
            graphics.tint = this.getTint(style);
        }
    }
}
...
addEventListener('keydown', ({ code }) => {
    for (const input of CosmosKeyboardInput.inputs) {
        input.keycodes.includes(code) && input.down(code);
    }
});

addEventListener('keyup', ({ code }) => {
    for (const input of CosmosKeyboardInput.inputs) {
        input.keycodes.includes(code) && input.up(code);
    }
});
