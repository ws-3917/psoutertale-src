...
export const translatorImages = {
    assets: [] as [string, CosmosImage | CosmosData][],
    content: new OutertaleTranslatable<Record<string, string>>({}),
    modes: ['default'] as string[],
    addAsset(key: string, image: CosmosImage | CosmosData) {
        translatorImages.assets.push([key, image]);
        translatorImages.content.defaults[key] = image.source;
    },
    updateMode(modeName: string) {
        const modeSources = translatorImages.content.getLanguage(modeName);
        for (const [key, image] of translatorImages.assets) {
            image.source = modeSources[key] ?? translatorImages.content.defaults[key];
        }
    }
};
