...
export default {
    a_starton: {
        ...
        eat_chip: [
            '<32>{#p/basic}* ¿Acabas de...\n* Comer la parte de mí que te había dado?',
            '<32>* No tengo nada que decirte...\n* ¡Vete!'
        ],
        eat_cream: ['<32>{#p/basic}* ¡Qué bueno que disfrutaste de tu Helado Soñado!\n* ¡Muy bien!'],
        genotext: {
            asriel1: () =>
                [['<25>{#p/asriel2}{#f/9}* Sólo sígueme la corriente...'], ['<25>{#p/asriel2}{#f/16}* Por aquí.']][
                Math.min(SAVE.flag.n.ga_asriel1++, 1)
                ],
            asriel2: () =>
                [
                    ["<25>{#p/asriel2}{#f/2}* Bueno, bueno... si es el Gran Papyrus aquí delante nuestra."],
                    ['<25>{#p/asriel2}{#f/3}* Bueno, bueno... aquí vamos de nuevo.']
                ][Math.min(SAVE.flag.n.killed_sans, 1)],
            asriel3: () =>
                [
                    ["<25>{#p/asriel2}{#f/1}* Vamos a presentarnos, ¿ok?"],
                    ['<25>{#p/asriel2}{#f/4}* Ya sabes como hacerlo.']
                ][Math.min(SAVE.flag.n.killed_sans, 1)],
            asriel4: ['<25>{*}{#p/asriel2}{#f/5}* ¡Howdy!{^5}{%}'],
            asriel5: ['<18>{*}{#p/papyrus}{#f/1}QUE NARICES- {%}'],
            asriel6: () =>
                [
                    [
                        '<25>{#p/asriel2}{#f/13}* ... $(name), tú...',
                        '<25>{#f/17}* ¿Tú crees que podrías tomar control a partir de ahora?',
                        "<25>{#f/15}* No es que haya nada mal conmigo, pero...",
                        "<25>{#f/16}* Simplemente creo que eres mejor en esto que yo.",
                        "<25>{#f/17}* Sí, eso es.\n* Eres mejor que yo en este tipo de cosas."
                    ],
                    ["<25>{#p/asriel2}{#f/16}* Vale, sí.\n* Yo, eh... te dejaré tener el control ahora."],
                    ['<25>{#p/asriel2}{#f/15}* Entonces, eh... ¿continuamos?'],
                    ['<25>{#p/asriel2}{#f/15}* ...']
                ][Math.min(SAVE.flag.n.ga_asriel6++, 3)],
            asriel9: ["<25>{#p/asriel2}{#f/8}* Psst, esperemos y veamos que hace."],
            asriel10: () =>
                [
                    [
                        '<25>{#p/asriel2}{#f/15}* Wow.\n* Ver a Papyrus así...',
                        "<25>{#f/16}* ... es realmente inesperado, ¿no crees?",
                        '<25>{#f/13}* Oh, $(name)...',
                        "<25>{#f/1}* Creo que nos vamos a divertir mucho."
                    ],
                    ['<25>{#p/asriel2}{#f/16}* Pobre Papyrus.']
                ][Math.min(SAVE.flag.n.ga_asriel10++, 1)],
            asriel17: () =>
                [["<25>{#p/asriel2}{#f/16}* Vaya... algunas personas simplemente no entienden."], ['<25>{#p/asriel2}{#f/4}* Tch.']][
                Math.min(SAVE.flag.n.ga_asriel17++, 1)
                ],
            asriel24: () =>
                [['<25>{#p/asriel2}{#f/4}* Que perdida de tiempo.'], ['<25>{#p/asriel2}{#f/3}* Huh.']][
            ...
            asriel26: () =>
                [
                    [
                        ...
                        '<25>{#f/1}* ... quedate detrás mio.'
                    ],
                    ['<25>{#p/asriel2}{#f/3}* A la ciudad nos vamos...']
                ][Math.min(SAVE.flag.n.ga_asriel26++, 1)],
            asriel28: () =>
                [
                    [
                        "<25>{#p/asriel2}{#f/6}* Okey, $(name).\n* La ciudad es toda tuya.",
                        "<25>{#f/7}* Mientras tanto, tengo que hacer algo que será importante más tarde.",
                        "<25>{#f/1}* Volveré antes de que te des cuenta."
                    ],
                    ['<25>{#p/asriel2}{#f/1}* Nos vemos atras de la ciudad.']
            ...
            asriel29: () =>
                [
                    SAVE.data.b.papyrus_secret
                        ? [
                            '<25>{#p/asriel2}{#f/2}* Je.\n* Je.\n* Je....',
                            "<25>{#f/10}* ... espera, ¿dónde esta Papyrus?",
                            '<25>{#f/10}* ...',
                            "<25>{#f/4}* Vaya, $(name), no pensé que lo matarías TAN rápido."
                        ]
                        : [
                            '<25>{#p/asriel2}{#f/2}* Je.\n* Je.\n* Je....',
                            "<25>{#f/1}* Ya era hora de que ese idiota pagara el precio de su piedad.",
                            '<25>{#f/13}* Vaya.\n* Él deseaba TANTO perdonarte.',
                            "<25>{#f/16}* Pero, seamos honestos con nosotros mismos...",
                            "<25>{#f/1}* Tenemos un pez más grande que freír."
                        ],
                    ['<25>{#p/asriel2}{#f/13}* Oh bueno.\n* El esqueleto volvió a morir en vano.'],
                    [
                        "<25>{#p/asriel2}{#f/13}* Sabes, dicen que la tercera es la vencida...",
                        '<25>{#f/16}* Lástima que él sólo reciba una.'
                    ],
                    [
                        "<25>{#p/asriel2}{#f/6}* Ya son cuatro veces que lo has matado.",
                        "<25>{#f/8}* Empiezo a pensar que disfrutas esto..."
                    ],
            ...
            asriel30: () => [
                ...
                '<25>{#f/1}* Hola, Asriel.',
                '<25>{#f/2}* ...',
                '<25>{#f/3}* Debemos hablar.',
                ...[
                    [
                        '<25>{#p/asriel2}{#f/6}* ¿Hablar?\n* ¿Sobre qué?',
                        '<25>{#f/6}* ¿Por qué estás aquí?',
                        "<25>{#f/7}* Sabes que de todas formas te me vas a morir.",
                        '<25>{#f/8}* Hablando de eso... {%15}'
                    ],
                    [
                        "<25>{#p/asriel2}{#f/8}* ¿Hablar?\n* No hagas perder mi tiempo.",
                        "<25>{#f/6}* SÉ que sólo estás usando un holograma.",
                        '<25>{|}{#p/asgore}{#f/5}* ¿Cómo hiciste- {%}',
                        '<25>{#p/asriel2}{#f/1}* Silencio.'
                    ]
                ][Math.min(SAVE.flag.n.ga_asriel30x, 1)]
            ],
            asriel30a: [
                '<25>{#p/asriel2}{#f/8}* ¿En serio?\n* ¿Un holograma?',
                '<25>{#f/6}* Sabía que eras un cobarde, pero esto es un nivel nuevo.'
            ],
            asriel30b: [
                '<25>{#p/asgore}{#f/1}* ¿No tienes nada mejor que hacer?',
                '<25>{#p/asriel2}{#f/8}* ...',
                '<25>{|}{#p/asgore}{#f/3}* Mira, hijo, yo simplemente- {%}',
                ...
                "<25>{#p/asriel2}{#f/8}* Agh, no lo digas como si de verdad te importara, papá.",
                '<25>{#p/asgore}{#f/5}* ...',
                '<25>{#p/asriel2}{#f/9}* Oh, perdón...\n* ¿Dije \"Papá?\"\n* Quise decir \"Asgore.\"',
                '<25>{#f/1}* Perdóname.',
                '<25>{#p/asgore}{#f/3}* ...\n* De verdad...',
                '<25>{#f/5}* Debes reconsiderar lo que estás haciendo, no por nuestro bien...',
                '<25>{#f/6}* ¡Sino por el tuyo!',
                '<25>{#p/asriel2}{#f/8}* ...',
                '<25>{#p/asriel2}{#f/7}* ... dame un respiro.',
                "<26>{#f/6}* Es obvio que sólo\n  estas aquí para tomarme el pelo.",
                '<25>{#p/asgore}{#f/3}* ...',
                '<25>{#p/asriel2}{#f/6}* ...',
                '<25>{#p/asgore}{#f/7}* ¡Debes considerar la gravedad de tus decisiones!',
                "<25>{#p/asriel2}{#f/15}* ¿O qué? ¿Voy a flotar en el espacio, para que nunca más me vuelvan a ver?",
                "<25>{#f/16}* Vamos $(name), hemos terminado aquí."
            ],
            asriel30c: ['<25>{*}{#p/asgore}{#f/8}* ¡Asriel, por favor!\n* ¡Solo quiero ayudar!{^999}'],
            asriel30d: () =>
                [
                    ['<25>{#p/asriel2}{#f/3}* Prepárate, $(name).', "<26>{#f/4}* Estos son los dominios de Undyne."],
                    ['<25>{#p/asriel2}{#f/4}* Acompañadme.']
                ][Math.min(SAVE.flag.n.ga_asriel30d++, 1)],
            papyrusSolo1a: [
                '<18>{#p/papyrus}{#f/31}¿SANS?\n¿ES ESE UN HUMANO?',
                "<18>{#f/5}LO ES, ¿VERDAD?",
                '<18>{#f/32}NYEH...\nUNDYNE FINALMENTE VA A...',
                "<18>{#p/papyrus}{#f/31}FINALMENTE PODRÉ UNIRME A LA GUARDIA REAL...",
                "<18>{#f/5}¿NO TE HACE ESO FELIZ?",
                "<25>{#p/asriel2}{#f/3}* No puedes seguir pretendiendo, Papyrus.\n* Se fué.",
                '<18>{|}{#p/papyrus}{#f/5}PERO- {%}',
                "<25>{#p/asriel2}{#f/3}* Ya se acabó.\n* Estas desperdiciando tu tiempo en él.", 
                "<18>{#p/papyrus}{#f/6}PERO NO PUEDE SER...\nSANS, ÉL...",
                '<18>{#f/31}ÉL PROMETIÓ...',
                "<25>{#p/asriel2}{#f/8}* Ese cabeza de hueso es la ULTIMA persona que confiaría en mantener una promesa.",
                "<26>{#f/9}* No es que yo sea mejor.", 
                '<18>{#p/papyrus}{#f/31}...',
                "<18>{#f/3}LO SIENTO.\nTENGO QUE IRME..."
            ],
            papyrusSolo2a: [
                '<18>{#p/papyrus}{#f/31}BUENO, ACABO DE VOLVER DE DONDE UNDYNE...',
                '<18>{#f/31}DICE QUE EL REY TIENE UNA OFERTA PARA TI.',
                '<25>{#p/asriel2}{#f/6}* ...',
                '<18>{#p/papyrus}{#f/3}SUS PALABRAS FUERON \"QUIERO VER A MI HIJO.\"',
                '<18>{#f/7}...',
                ...
                '<18>{#p/papyrus}{#f/7}... ¡¡AGH!!',
                '<18>{#p/papyrus}{#f/4}EN CUANTO \nA TI , HUMANO...',
                "<18>{#f/7}NO CREAS QUE NO VEO LO QUE ESTA PASANDO.",
                "<18>{#f/7}¡ES OBVIO QUE TÚ ERES EL QUE MANDA!",
                '<25>{#p/asriel2}{#f/8}* Que observador.',
                '<25>{#f/7}* Entonces supongo que deberíamos admitir derrota aquí y ahora, ¿no?',
                '<18>{#p/papyrus}{#f/31}...',
                '<25>{#p/asriel2}{#f/4}* Dejame ser claro en esto.\n* Yo admiro el esfuerzo.',
                "<25>{#f/3}* Pero tenemos nuestros planes.",
                "<18>{#p/papyrus}{#f/4}PARA QUE SEPAS, UNDYNE PROBABLEMENTE NOS ESTE MIRANDO.",
                '<25>{#p/asriel2}{#f/3}* ¿Y tu punto es?',
                ...
                '<18>{#p/papyrus}{#f/7}¡¡¡LO QUE SEA!!!'
            ],
            papyrusSolo3: ['<25>{#p/asriel2}{#f/3}* Hola.'],
            papyrusSolo3a: () => [
                '<18>{#p/papyrus}{#f/31}SABES...',
                '<18>{#f/31}OÍ A LA DRA. ALPHYS HABLANDO...',
                '<18>{#f/5}Y DIJO ALGO COMO \"¿VIAJAR HACIA ATRÁS EN EL TIEMPO?\"',
                "<18>{|}{#f/32}{#x1}NO PUEDO ESTAR SEGURO, PERO ESO SONABA COMO- {%}",
                '<25>{#p/asriel2}{#f/6}* No.',
                '<18>{|}{#p/papyrus}{#f/6}PERO DIJO QUE TÚ SERÍAS CAPAZ DE- {%}',
                ...
                '<18>{#p/papyrus}{#f/31}PERO, SI VOSOTROS DE VERDAD PODÉIS BORRAR LO QUE PASÓ...',
                '<18>{#f/5}¿ENTONCES POR QUÉ NO?',
                "<18>{#f/31}Y-Y, EN LA SIGUIENTE LÍNEA TEMPORAL... TOMARÉ SU LUGAR.",
                "<18>{#f/3}ENTONCES ÉL NO TENDRÍA QUE MORIR, ¿VERDAD?",
                "<25>{#p/asriel2}{#f/6}* ...\n* Créeme, ya vi esa línea temporal.",
                "<25>{#f/7}* Es ABURRIDA.",
                '<18>{#p/papyrus}{#f/3}...',
                '<18>{#f/6}¿PERO Y SI TE ENSEÑO ESTE PUZZLE?',
                '<18>{#f/32}QUIZÁS AYUDA A CALMAR TU ABURRIMIENTO...',
                '<25>{#p/asriel2}{#f/15}* ...',
                '<25>{#p/asriel2}{#f/15}* Si te hace sentir mejor, supongo.',
                '<18>{#p/papyrus}OH... ¡OH!',
                "<18>{#f/0}¡¡ESO ES GENIAL!!",
                "<18>{#f/0}¡¡YA ESTÁS CAMBIANDO DE IDEA!!",
                '<25>{#p/asriel2}{#f/8}* ...',
                '<18>{#p/papyrus}{#f/6}...',
                '<18>{|}{#f/5}BUENO, AQUÍ ESTÁN LAS REGLAS DE- {%}',
                '<25>{#p/asriel2}{#f/7}* Ya conocemos las reglas, idiota.',
                '<18>{#p/papyrus}{#f/31}... OH...',
                '<18>{#f/6}EH, ¡¡BIEN!!\nSIN NADA MÁS QUE HACER...',
                "<18>{#f/9}¡¡VAMOS A VER QUE NÚMERO ALEATORIO NOS TOCA!!"
            ],
            papyrusSolo4a: [
                ...
                '<18>{#p/papyrus}{#f/31}...',
                '<18>{#f/31}¿POR QUÉ?',
                '<18>{#f/31}¿POR QUÉ HARÍAS ESTO?',
                "<18>{#f/3}NO SE SUPONE QUE LOS MONSTRUOS SEAN ASÍ...",
                "<18>{#f/5}¿DÓNDE ESTÁ TU AMOR?\n¿TÚ COMPASIÓN?",
                '<18>{#f/31}TU... PIEDAD...',
                '<25>{#p/asriel2}{#f/2}* ...\n* Oh, dulce niño estelar...',
                '<25>{#f/1}* Perdí esas cosas hace un LARGO tiempo.',
                "<18>{#p/papyrus}{#f/31}PERO...\nNO LO ENTIENDO...",
                '<18>{#f/5}¿CÓMO PUEDE UN MONSTRUO TAN PURO DE MENTE...',
                '<18>{#f/31}... SER CAMBIADO TOTALMENTE AL LADO OSCURO?',
                '<25>{#p/asriel2}{#f/1}* ¿De verdad quieres saberlo?',
                '<18>{#p/papyrus}{#f/3}...',
                '<18>{#f/3}SÍ...',
                '<25>{#p/asriel2}{#f/10}* ¿Pero quieres saberlo mucho, muchísimo?',
                '<18>{#p/papyrus}{#f/31}SÍ.',
                '<25>{#p/asriel2}{#f/3}* Dilo más alto.',
                '<18>{#p/papyrus}{#f/5}¡SÍ!',
                '<26>{#p/asriel2}{#f/1}* Con una exobaya encima.',
                '<18>{#p/papyrus}{#f/7}¡SÍ!\n¡CON UNA EXOBAYA ENCIMA, MALDICIÓN!',
                '<25>{#p/asriel2}{#f/1}* Je je je...',
                "<25>{#f/1}* Está bien, te lo diré.",
                "<25>{#f/15}* De hecho, solo será una palabra...",
                '<18>{#p/papyrus}{#f/4}OH POR DIOS, SOLO DILO DE UNA VEZ...'
            ],
            papyrusSolo4b: [
            ...
            papyrusSolo4d: [
                "<18>{#p/papyrus}{#f/7}NO, ESTÁS EQUIVOCADO.",
                "<18>{#f/7}TÚ ERES QUIEN HA ESTADO INTENTANDO QUE ME VENGA ABAJO.",
                "<18>{#f/7}TÚ ERES QUIEN ME DICE MENTIRA TRAS MENTIRA.",
                '<18>{#f/9}PERO YO, PAPYRUS...',
                '<18>{#f/9}FINALMENTE ENTIENDO LA {@fill=#f00}VERDAD VERDADERA{@fill=#fff}.',
                "<25>{#p/asriel2}{#f/13}* ¿Oh?\n* ¿Y cuál es esa?"
            ],
            papyrusSolo4e: ["<18>{#p/papyrus}{#f/34}TÚ NO ERES {@fill=#f00}ASRIEL{@fill=#fff}."],
            papyrusSolo4f: [
                '<18>{#f/31}{@fill=#f00}ASRIEL{@fill=#fff} NUNCA ACTUARÍA ASÍ.',
                '<18>{#f/5}{@fill=#f00}ASRIEL{@fill=#fff} ERA UN ALMA BONDADOSA.',
                '<18>{#f/5}{@fill=#f00}ASRIEL{@fill=#fff} CREÍA EN LA GENTE...',
                '<18>{#f/31}ÉL CREÍA EN LA HUMANIDAD MÁS QUE NADIE.',
                '<18>{#f/4}TÚ, POR EL OTRO LADO...',
                '<18>{#f/7}¡SOLO QUIERES USARLE PARA TUS PROPIOS MEDIOS!',
                "<18>{#f/4}E IGUALMENTE, NO ME IMPORTA LO QUE TENGAS QUE DECIR.",
                '<18>{#f/9}AÚN TENGO ESPERANZA EN ESE HUMANO.',
                "<25>{#p/asriel2}{#f/8}* Bueno, si tienes tanta fé en él...",
                '<25>{#f/7}* Demuestra que me equivoco.',
                "<25>{#f/3}* Dejaré que le enfrentes, uno a uno.",
                "<25>{#f/3}* Si te perdona, admitiré que me equivoco.",
                '<25>{#f/4}* Pero si te mata, que inevitablemente pasará...',
                "<25>{#f/1}* Verás que tenía razón, y que ÉL murió para nada.",
                '<25>{#f/1}* ¿Qué te parece?',
                '<18>{#p/papyrus}{#f/9}...\nYO ACEPTO.',
                '<25>{#p/asriel2}{#f/3}* Espléndido.',
                '<25>{#f/4}* Hasta nunca.'
            ]
        },
        houz: () =>
        ...
        papcomputer5: () =>
            world.runaway
                ...
                : SAVE.data.n.plot === 72
                    ? ['PERDÓN', "BUT WE'RE", 'DESCONECTADOS', 'LOL']
                    : ['REINICIAR', 'MENSAJES', 'AJUSTES', 'LOG OUT'],
        papcouch0: () =>
            SAVE.data.b.svr
        ...
        paproom5: ['<18>{#p/papyrus}{#f/4}(SUSPIRO...)'],
        papdate0: () => [
        ...
        papdate4: pager.create(
            ...
            () => ['<18>{#p/papyrus}¿LISTO?', choicer.create('* (¿Qué dices?)', 'Sí', 'No')]
        ),
        ...
        papdate15a: ['<15>{#p/papyrus}{#f/12}¿¿¿E-ENSERIO???', '<15>{#f/11}¡¡¡WOWIE!!!'],
